...
static const char *gProgramName = ProgramName;
 volatile mDNSBool gStopNow=0;


#if 0 //by ziqiang,we don't need signal
...
static const char *gServiceName      = "RalinkAP Config page";
...
/* Daemon relative */
cyg_handle_t responder_thread_h ;
cyg_thread    responder_thread;
static char  responder_stack[8192];//[ STACK_SIZE ];//change dhcpd_stack size:8192->5*1024

extern int Responder_main(int argc, char **argv);

int  mDNS_responder_init(int argc,char * argv[])
{
	cyg_thread_create
	(
		8,									/* scheduling info (eg pri) */
		(cyg_thread_entry_t *)Responder_main, /* entry function */
		0,									/* entry data */
		"mDNS_Responder",						/* optional thread name */
		responder_stack,						/* stack base, NULL = alloc */
		sizeof(responder_stack),//STACK_SIZE,							/* stack size, 0 = default */
		&responder_thread_h,					/* returned thread handle */
		&responder_thread						/* put thread here */
	);
	cyg_thread_resume(responder_thread_h); 
...
#if 0 // TBD:XXXX no file support now
...
int Responder_main(int argc, char **argv)
{
  ...
  // ParseArguments(argc, argv);
	...
	{
		gServiceText[gServiceTextLen] = strlen("LuckyDog");
		mDNSPlatformMemCopy(gServiceText+gServiceTextLen+1, "LuckyDog", gServiceText[gServiceTextLen]);
		gServiceTextLen += 1 + gServiceText[gServiceTextLen];
	}
    ...
    while (!gStopNow)
    {
      ...
      //  debugf("select(%d, %d.%06d)", nfds, timeout.tv_sec, timeout.tv_usec);
        result = select(nfds, &readfds, NULL, NULL, &timeout);
	//diag_printf("\nSelect return\n");
        if (result < 0)
        {
        diag_printf("[mDNS]%s:%d Select ERROR\n",__FUNCTION__,__LINE__);
#if 0 //by ziqiang    NO Signal support now
			...
			;//diag_printf("[mDNS]%s:%d receive timeout\n",__FUNCTION__,__LINE__);
        	}
	else  if (result >0)
        {
    ...
    diag_printf("mDNS Responder Exiting\n");
