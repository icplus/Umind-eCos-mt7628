...
extern int wifi_up;
	if(wifi_up){
		...
		int apcli_flag=0;
		netif_flag_get("apcli0", &apcli_flag);
		if((opmode == 2 || opmode ==3)&&(!(apcli_flag&IFF_UP)))
			{
			diag_printf("%s %d:Trigger force up apcli0 \n",__FUNCTION__,__LINE__);
			netif_up("apcli0");//up apcli0, so ApCliAutoConnect can do connect again
			}
	}
