...
int Wireless_down()
{
	...
	for (i = 1; i < 4; i++)//make sure all mbss can be down
	{
		...
		diag_printf("%s:ra%d down\n",__FUNCTION__,i);
	}
	
	result = netif_down("ra0");
	diag_printf("%s:ra0 down\n",__FUNCTION__);
	if (!result)
		return -1;
...
int Wireless_start()
{
    ...
    else if( result && (opmode == 2) )
    {
        int dhcp_dis=0;
        CFG_get( CFG_LAN_DHCPD_EN, &dhcp_dis);
        if(dhcp_dis != 1)
		{
            sprintf(temp_cfg,"%s, %s",bridge_cfg,"apcli0");
            sprintf(bridge_cfg,"%s",temp_cfg);
