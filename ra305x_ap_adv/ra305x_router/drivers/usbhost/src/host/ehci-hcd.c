...
//struct QH qh_list_global;
//struct QH *qh_list = NULL;
//struct QH alloc_qh;
//struct qTD alloc_td[3];
...
extern int rt3xxx_ehci_probe(void);//Jody
 ...
 */
static int handshake (struct ehci_hcd *ehci, void *ptr,
...
static int handshake_on_error_set_halt(struct ehci_hcd *ehci, void *ptr,
				       u32 mask, u32 done, int usec)
{
  #if 0 //Jody
...
static int ehci_reset (struct ehci_hcd *ehci)
{
	...
	ehci->next_statechange = 100;
	retval = handshake (ehci, &ehci->regs->command,
			    CMD_RESET, 0, 250 * 1000);

	if (ehci->has_hostpc) {
		ehci_writel(ehci, USBMODE_EX_HC | USBMODE_EX_VBPS,
			(u32 *)(((u8 *)ehci->regs) + USBMODE_EX));
		ehci_writel(ehci, TXFIFO_DEFAULT,
			(u32 *)(((u8 *)ehci->regs) + TXFILLTUNING));
	}
	if (retval)
		return retval;
...
#include "ehci-hub.c"//Jody
#include "ehci-lpm.c"
#include "ehci-mem.c"
#include "ehci-q.c"
//#include "ehci-sched.c"//Jody
...
static void ehci_shutdown(struct usb_hcd *hcd)
{
	...
	//del_timer_sync(&ehci->watchdog); //Jody
	//del_timer_sync(&ehci->iaa_watchdog);//Jody

	//spin_lock_irq(&ehci->lock);//Jody
...
static int ehci_init(struct usb_hcd *hcd)
{
	...
	//spin_lock_init(&ehci->lock);//Jody

	/*
	 * keep io watchdog by default, those good HCDs could turn off it later
	 */
	ehci->need_io_watchdog = 1;
	//init_timer(&ehci->watchdog);//Jody
	ehci->watchdog.function = ehci_watchdog;
	ehci->watchdog.data = (unsigned long) ehci;

	//init_timer(&ehci->iaa_watchdog);//Jody
	...
	if ((retval = ehci_mem_init(ehci, 0)) < 0)
		return retval;
...
// this is deeply broken on almost all architectures
	...
	//ehci->last_periodic_enable = ktime_get_real();//Jody
...
static int ehci_urb_dequeue(struct usb_hcd *hcd, struct urb *urb, int status)
{
	...
	//spin_lock_irqsave (&ehci->lock, flags);//Jody
...
rescan:
	...
	case QH_STATE_UNLINK_WAIT:
idle_timeout:
		//spin_unlock_irqrestore (&ehci->lock, flags);//Jody
...
err_debug:
#endif
	//clear_bit(USB_EHCI_LOADED, &usb_hcds_loaded);//Jody
	rt3xxx_ehci_probe();
	return retval;
}
...
//	debug("dev=%p, pipe=%lx, buffer=%p, length=%d, req=%p\n", dev, pipe, buffer, length, req);
	if (req != NULL)
		diag_printf("ehci_submit_async(): req=%u (%#x), type=%u (%#x), value=%u (%#x), index=%u\n",
	...
	qh->qh_link = EHCI_virt_to_bus(cpu_to_le32((uint32_t)qh_list | QH_LINK_TYPE_QH));
