...
retry:
	...
	//ohci->next_statechange = jiffies + STATECHANGE_DELAY;//Jody
	//spin_unlock_irq (&ohci->lock);
...
static irqreturn_t ohci_irq (struct usb_hcd *hcd)
{
	...
	if (ints & OHCI_INTR_UE) {
		...
		if (quirk_nec(ohci)) {
			...
			//schedule_work (&ohci->nec_work);//Jody
		} else {
			disable (ohci);
...
*/
	static int ohci_rt3xxx_start(struct usb_hcd *hcd)
	{
		...
		if ((ret = ohci_run(ohci)) < 0) {
			diag_printf("can't start %s", hcd->self.bus_name);
			ohci_stop(hcd);
			return ret;
		}
...
extern ohci_hub_status_data (struct usb_hcd *hcd, char *buf);
extern int ohci_hub_control (struct usb_hcd *hcd, u16 typeReq,u16 wValue,u16 wIndex,char *buf,u16 wLength); 
...
static int  ohci_hcd_mod_init(void)
{
	...
	diag_printf("enter:%s, hcd_name:%s\n", hcd_name);
