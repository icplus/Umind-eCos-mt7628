		...
		int any_connected, int rhsc_status)
{
	...
	//if (ohci_readl(ohci, ohci->regs->cmdstatus) & OHCI_INTR_RHSC)
	if (ohci_readl(ohci, 0x101c1008) & OHCI_INTR_RHSC)//Jody ohci->regs->cmdstatus:0x101c1008
		return 0;
	...
	ohci_writel(ohci, OHCI_INTR_RHSC, 0x101c1010);//Jody ohci->regs->intrenable:0x101c1010
	return 0;
}
...
ohci_hub_status_data (struct usb_hcd *hcd, char *buf)
{
	...
	if ((ohci->flags & 0x01)  //OHCI_QUIRK_AMD756:0x01
...
ohci_hub_descriptor (
	...
	if (rh & RH_A_NOCP)		/* no overcurrent reporting? */
	    ...
	    temp |= 0x0008;
	desc->wHubCharacteristics = ( __u16)cpu_to_hc16(ohci, temp);
