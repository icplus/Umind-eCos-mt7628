...
enum DONGLE_MSGQ_CMD_E
{
    DONGLE_CMD_STOP = 1,  // Comming from upper layer
    DONGLE_CMD_HANGUP,    // Comming from lower layer
    DONGLE_CMD_RECONNECT,    // Comming from lower layer
    DONGLE_CMD_CALLMGR_RECONNECT,
    DONGLE_CMD_TIMER
};
...
#define DONGLE_EVENT_RX 0x01
#define DONGLE_EVENT_QUIT 0x02
#define DONGLE_SEND_QUEUE_SIZE 512
...
//int lf_SendPppFrame(l2tp_session *ses, unsigned char const *buf, int len);
...
int encaps_dongle_usermode(struct l2tp_if *ifp, struct mbuf *m0);  
