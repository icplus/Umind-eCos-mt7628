...
int32_t MUSB_BitsGet(const uint8_t *pBuffer, uint8_t bOffset, uint8_t bCount, uint32_t *pdwResult)
{
    uint8_t bTopByte;
    ...
    if (pBuffer && pdwResult)
    {
        if ((bTopBit <= 32) && (bOffset < 8))
        {
            ...
            if (bTopBits)
            {
                dwTemp = pBuffer[bByte-- ] & (~(uint8_t)(0xff << bTopBits));
            }
            else
            {
...
int32_t MUSB_BitsSet(uint8_t *pBuffer, uint8_t bOffset, uint8_t bCount, uint32_t dwValue)
{
    uint8_t bTopByte;
