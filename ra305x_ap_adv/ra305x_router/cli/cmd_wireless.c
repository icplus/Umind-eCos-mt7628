...
int CmdIwpriv(int argc, char *argv[])
{
  ...
  diag_printf("123\n");
	...
	if ( found == 1 )
	{
		...
		rt28xx_sta_ioctl(sc, cmd, (caddr_t)&param);
#endif
...
//mjh
//static char TempString[8192];
int Cmd_mjh(int argc, char *argv[])
{
	int cmd;
	cyg_netdevtab_entry_t *t;
	struct eth_drv_sc *sc;
  ...
  //diag_printf("123\n");
	...
	if ( found == 1 )
	{
		...
		if (param.u.data.length != original_length)
		{
			...
			//diag_printf("%s", TempString);  //jie shou 0~2049 2050 ge char
		}
		
		mjh_str1 = TempString;
		mjh_str2 = &TempString[2050];
		mjh_str3 = &TempString[4100];
		mjh_str4 = &TempString[6150];
		mjh_str5 = &TempString[8210];
		mjh_str6 = &TempString[10260];
		mjh_str7 = &TempString[12310];
		...
		//TempString[2050] = '\0';   //2050 printf not a string
		//diag_printf("test = %s\n",mjh_str);
		//free(TempString);
