		...
		struct ntfs_device_operations *dops, void *priv_data)
{
    ...
    ntfs_log_debug("enter ntfs_device_alloc\n");
    err = usb_io_lookup(name, &dev_h);
    if (err != ENOERR)
    	{
        diag_printf("file test:dev not fount\n");return NULL;}

    disk_channel *tempchan = ((cyg_devtab_entry_t*)dev_h)->priv;
    if(0x07 != tempchan->partition->type)
        return NULL;
	...
	if (dev) {
		...
		dev->d_ops = dops;
		dev->d_state = state;
		dev->d_private = (cyg_devtab_entry_t*) malloc (sizeof(cyg_devtab_entry_t));
        if(dev->d_private == NULL)
        {
            free(dev);
            return NULL;
        }
        memcpy(dev->d_private,dev_h,sizeof(cyg_devtab_entry_t));

        disk_channel *chan = (disk_channel *) malloc (sizeof(disk_channel));
        if(chan == NULL)
        {
            free((cyg_devtab_entry_t*)(dev->d_private));
            free(dev);
            return NULL;
        }
        memcpy(chan,((cyg_devtab_entry_t*)dev_h)->priv,sizeof(disk_channel));
        ((cyg_devtab_entry_t*)(dev->d_private))->priv = chan;

        dev->sector_size = chan->info->block_size;
	}
	return dev;
}
