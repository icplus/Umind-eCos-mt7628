		...
		ntfs_mount_flags flags)
{
	...
	if (flags & NTFS_MNT_RDONLY)
		NVolSetReadOnly(vol);
	NVolClearReadOnly(vol);//added by zhiqiang
    ...
    char *tembuf = ntfs_malloc(dev->sector_size);
	br = ntfs_pread(dev, 0, dev->sector_size, tembuf);
	if (br != dev->sector_size) {
		if (br != -1)
			errno = EINVAL;
		if (!br)
    ...
    memcpy(bs,tembuf,sizeof(NTFS_BOOT_SECTOR));
    //diag_dump_buf_with_offset(bs,512,bs);
    free(tembuf);
...
/**
 * ntfs_mount_vol - open ntfs volume
 ...
 */
ntfs_volume *ntfs_mount_vol(const char *name __attribute__((unused)),
...
/**
 * ntfs_umount_vol - close ntfs volume
 ...
 */
int ntfs_umount_vol(ntfs_volume *vol, const BOOL force __attribute__((unused)))
{
	struct ntfs_device *dev;
	int ret;
