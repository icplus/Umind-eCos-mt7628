...
#define DATA_CACHE_SIZE 1024*128
/**
...
struct _ntfs_volume {
    ...
    u8 *data_buf;
    u64 buf_len;//data length filled in buf
    u64 pos;//buf start pos
...
extern ntfs_volume *ntfs_mount_vol(const char *name, ntfs_mount_flags flags);
extern int ntfs_umount_vol(ntfs_volume *vol, const BOOL force);
