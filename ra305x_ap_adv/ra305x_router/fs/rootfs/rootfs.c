...
// rootfs_node structure contains the following fields:
//
...
// a chain of fixed size rootfs_dirent structures. These contain the
// following fields:
//
...
//#define CYGNUM_RAMFS_BLOCK_SIZE  256
//added by zhiqiang
#define CYGPKG_FS_RAM_SIMPLE 1
#define CYGNUM_RAMFS_DIRENT_SIZE 32
#define CYGNUM_RAMFS_REALLOC_INCREMENT 256
...
#define RAMFS_NODES_PER_BLOCK (CYGNUM_RAMFS_BLOCK_SIZE/sizeof(rootfs_node))

// The number of indirect pointers that can be stored in a single data block
#define RAMFS_INDIRECT_PER_BLOCK (CYGNUM_RAMFS_BLOCK_SIZE/sizeof(rootfs_block *))

// The number of directory entries that can be stored in a single data block
#define RAMFS_DIRENT_PER_BLOCK  (CYGNUM_RAMFS_BLOCK_SIZE/sizeof(rootfs_dirent))
...
static int rootfs_mount    ( cyg_fstab_entry *fste, cyg_mtab_entry *mte );
static int rootfs_umount   ( cyg_mtab_entry *mte );
static int rootfs_open     ( cyg_mtab_entry *mte, cyg_dir dir, const char *name,
                             int mode,  cyg_file *fte );
static int rootfs_unlink   ( cyg_mtab_entry *mte, cyg_dir dir, const char *name );
static int rootfs_mkdir    ( cyg_mtab_entry *mte, cyg_dir dir, const char *name );
static int rootfs_rmdir    ( cyg_mtab_entry *mte, cyg_dir dir, const char *name );
static int rootfs_rename   ( cyg_mtab_entry *mte, cyg_dir dir1, const char *name1,
                             cyg_dir dir2, const char *name2 );
static int rootfs_link     ( cyg_mtab_entry *mte, cyg_dir dir1, const char *name1,
                             cyg_dir dir2, const char *name2, int type );
static int rootfs_opendir  ( cyg_mtab_entry *mte, cyg_dir dir, const char *name,
                             cyg_file *fte );
static int rootfs_chdir    ( cyg_mtab_entry *mte, cyg_dir dir, const char *name,
                             cyg_dir *dir_out );
static int rootfs_stat     ( cyg_mtab_entry *mte, cyg_dir dir, const char *name,
                             struct stat *buf);
static int rootfs_getinfo  ( cyg_mtab_entry *mte, cyg_dir dir, const char *name,
                             int key, void *buf, int len );
static int rootfs_setinfo  ( cyg_mtab_entry *mte, cyg_dir dir, const char *name,
                             int key, void *buf, int len );

// File operations
static int rootfs_fo_read      (struct CYG_FILE_TAG *fp, struct CYG_UIO_TAG *uio);
static int rootfs_fo_write     (struct CYG_FILE_TAG *fp, struct CYG_UIO_TAG *uio);
static int rootfs_fo_lseek     (struct CYG_FILE_TAG *fp, off_t *pos, int whence );
static int rootfs_fo_ioctl     (struct CYG_FILE_TAG *fp, CYG_ADDRWORD com,
                                CYG_ADDRWORD data);
static int rootfs_fo_fsync     (struct CYG_FILE_TAG *fp, int mode );        
static int rootfs_fo_close     (struct CYG_FILE_TAG *fp);
static int rootfs_fo_fstat     (struct CYG_FILE_TAG *fp, struct stat *buf );
static int rootfs_fo_getinfo   (struct CYG_FILE_TAG *fp, int key, void *buf, int len );
static int rootfs_fo_setinfo   (struct CYG_FILE_TAG *fp, int key, void *buf, int len );

// Directory operations
static int rootfs_fo_dirread      (struct CYG_FILE_TAG *fp, struct CYG_UIO_TAG *uio);
static int rootfs_fo_dirlseek     (struct CYG_FILE_TAG *fp, off_t *pos, int whence );
...
FSTAB_ENTRY( rootfs_fste, "rootfs", 0,
...
static cyg_fileops rootfs_fileops =
{
    rootfs_fo_read,
    rootfs_fo_write,
    rootfs_fo_lseek,
...
static cyg_fileops rootfs_dirops =
{
    rootfs_fo_dirread,
...
struct rootfs_node;
typedef struct rootfs_node rootfs_node;

struct rootfs_dirent;
typedef struct rootfs_dirent rootfs_dirent;

#ifndef CYGPKG_FS_RAM_SIMPLE

typedef cyg_uint8 rootfs_block[CYGNUM_RAMFS_BLOCK_SIZE];
...
struct rootfs_node
{
    ...
    rootfs_block         *direct[CYGNUM_RAMFS_BLOCKS_DIRECT];
#endif
#if  CYGNUM_RAMFS_BLOCKS_INDIRECT1 > 0
    // Single level indirection
    rootfs_block         **indirect1[CYGNUM_RAMFS_BLOCKS_INDIRECT1];
#endif
#if  CYGNUM_RAMFS_BLOCKS_INDIRECT2 > 0
    // Two level indirection
    rootfs_block         ***indirect2[CYGNUM_RAMFS_BLOCKS_INDIRECT2];
#endif
...
struct rootfs_dirent
{
    rootfs_node          *node;          // pointer to node
...
struct rootfs_dirsearch
{
    rootfs_node          *dir;           // directory to search
    const char          *path;          // path to follow
    rootfs_node          *node;          // Node found
    const char          *name;          // last name fragment used
    int                 namelen;        // name fragment length
    cyg_bool            last;           // last name in path?
};

typedef struct rootfs_dirsearch rootfs_dirsearch;

//==========================================================================
// Forward defs

static int del_direntry( rootfs_node *dir, const char *name, int namelen );
...
__externC rootfs_block CYGPKG_FS_RAM_BLOCKS_ARRAY_NAME[CYGNUM_FS_RAM_BLOCKS_ARRAY_SIZE];

// Translate into a usable common name
#define rootfs_block_array CYGPKG_FS_RAM_BLOCKS_ARRAY_NAME

# else

// Array is defined here

static rootfs_block cyg_rootfs_block_array[CYGNUM_FS_RAM_BLOCKS_ARRAY_SIZE];

#define rootfs_block_array cyg_rootfs_block_array

# endif

// Pointer to list of free blocks
static rootfs_block *block_free_list = NULL;
...
static void block_init(void)
{
    ...
    if( !initialized )
    {
        ...
        {
            rootfs_block *b = &rootfs_block_array[i];
            *(rootfs_block **)b = block_free_list;
            block_free_list = b;
        }
        initialized = true;
    }
...
static rootfs_block *block_alloc(void)
{
    rootfs_block *b;
    ...
    // and advance list
    if( b != NULL )
        block_free_list = *(rootfs_block **)b;
...
static void block_free( rootfs_block *b )
{
#ifdef CYGPKG_FS_RAM_BLOCKS_ARRAY
...
static int findbuffer_node( rootfs_node  *node,  // node pointer
...
static int freebuffer_node( rootfs_node *node )
{
    if( node->data != NULL )
    {
        free( node->data );
...
static int findbuffer_node( rootfs_node  *node,
                            off_t pos,
                            cyg_uint8 **buffer,
                            size_t *size,
                            cyg_bool alloc)
...
static void freeblock_list( rootfs_block *blocks[],int nblocks )
{
    int i;
    for( i = 0; i < nblocks ; i++ )
    {
...
static int freebuffer_node( rootfs_node *node )
{
...
#if CYGNUM_RAMFS_BLOCKS_INDIRECT1 > 0
    {
        ...
        for( i = 0; i < CYGNUM_RAMFS_BLOCKS_INDIRECT1 ; i++ )
        {
            ...
            {
                freeblock_list( (rootfs_block **)node->indirect1[i], RAMFS_INDIRECT_PER_BLOCK );
                block_free( (rootfs_block *)node->indirect1[i] );
...
#if CYGNUM_RAMFS_BLOCKS_INDIRECT2 > 0
    {
        ...
        for( i = 0; i < CYGNUM_RAMFS_BLOCKS_INDIRECT2 ; i++ )
        {
            ...
            {
                rootfs_block ***b = node->indirect2[i];
                int j;
                for( j = 0; j < RAMFS_INDIRECT_PER_BLOCK ; j++ )
                {
                    if( b[j] != NULL )
                    {
                        freeblock_list( (rootfs_block **)b[j], RAMFS_INDIRECT_PER_BLOCK );
                        block_free( (rootfs_block *)b[j] );
                        b[j] = NULL;
                    }
                }
                block_free( (rootfs_block *)node->indirect2[i] );
...
static rootfs_node *alloc_node( mode_t mode )
{
#ifdef CYGPKG_FS_RAM_SIMPLE
    rootfs_node *node = malloc( sizeof( rootfs_node ) );

    if( node == NULL )
        return NULL;
    
#else
    rootfs_block *b = block_alloc();
    rootfs_node *node;
    ...
    memset( node, 0, sizeof(rootfs_node) );
...
static void free_node( rootfs_node *node )
{
#ifdef CYGPKG_FS_RAM_SIMPLE    
...
static int dec_refcnt( rootfs_node *node )
{
    int err = ENOERR;
    node->refcnt--;
...
static int dec_nlink( rootfs_node *node )
{
    node->refcnt++;
                       ...
                       )
{
    ...
    while( namelen > 0 )
    {
        ...
        for(;;)
        {
            ...
            if( size < sizeof(rootfs_dirent) || d->inuse )
            {
                pos += sizeof(rootfs_dirent);
                continue;
            }
...
static rootfs_dirent *find_direntry( rootfs_node *dir, const char *name, int namelen )
{
    rootfs_dirent *first = NULL;
    off_t pos = 0;
    int err;
    //diag_printf("find_direntry name=%s\n",name);
    ...
    while( pos < dir->size )
    {
	  ...
	  err = findbuffer_node( dir, pos, &buf, &size, false );
            ...
            d = (rootfs_dirent *)buf;
            //diag_printf("d=NULL\n");
            if( size < sizeof(rootfs_dirent) || !d->inuse || !d->first )
            {
                pos += sizeof(rootfs_dirent);
		if ( pos < dir->size )
		  continue;
...
static int del_direntry( rootfs_node *dir, const char *name, int namelen )
{
    rootfs_dirent *d = find_direntry( dir, name, namelen );
...
static void init_dirsearch( rootfs_dirsearch *ds,
                            rootfs_node *dir,
                            const char *name)
{
    ds->dir      = dir;
...
static int find_entry( rootfs_dirsearch *ds )
{
    rootfs_node *dir = ds->dir;
    const char *name = ds->path;
    const char *n = name;
    char namelen = 0;
    rootfs_dirent *d;
    //diag_printf("find_entry name=%s\n",name);
...
static int rootfs_find( rootfs_dirsearch *d )
{
    int err;
    //diag_printf("enter rootfs_find,name=%s\n",d->path);
    ...
    for(;;)
    {
        ...
        //if(d->node == NULL){diag_printf("rootfs_find d->node=NULL\n");return ENOERR;}
        d->dir = d->node;
        d->path += d->namelen;
...
static int rootfs_pathconf( rootfs_node *node, struct cyg_pathconf_info *info )
{
    int err = ENOERR;
...
static int rootfs_umount   ( cyg_mtab_entry *mte )
{
    rootfs_node *root = (rootfs_node *)mte->root;
                             ...
                             int mode,  cyg_file *file )
{
    ...
    rootfs_dirsearch ds;
    rootfs_node *node = NULL;
    int err;

    init_dirsearch( &ds, (rootfs_node *)dir, name );
...
static int rootfs_mkdir    ( cyg_mtab_entry *mte, cyg_dir dir, const char *name )
{
    ...
    rootfs_node *node = NULL;
    int err;
    //diag_printf("enter rootfs_mkdir,name=%s\n",name);
    init_dirsearch( &ds, (rootfs_node *)dir, name );
    ...
    if( err == ENOENT )
    {
        if( ds.last )
        {
            ...
            int doterr=0, dotdoterr=0, direrr=0;
...
static int rootfs_rename   ( cyg_mtab_entry *mte, cyg_dir dir1, const char *name1,
                             cyg_dir dir2, const char *name2 )
{
    rootfs_dirsearch ds1, ds2;
    int err;

    init_dirsearch( &ds1, (rootfs_node *)dir1, name1 );
    
    err = rootfs_find( &ds1 );

    if( err != ENOERR ) return err;

    init_dirsearch( &ds2, (rootfs_node *)dir2, name2 );
                             ...
                             cyg_file *file )
{
    rootfs_dirsearch ds;
    int err;
    //diag_printf("enter rootfs_opendir,name=%s\n",name);
    init_dirsearch( &ds, (rootfs_node *)dir, name );
    ...
    file->f_type        = CYG_FILE_TYPE_FILE;
    file->f_ops         = &rootfs_dirops;
    file->f_offset      = 0;
    file->f_data        = (CYG_ADDRWORD)ds.node;
    file->f_xops        = 0;
    //diag_printf("exit rootfs_opendir,name=%s\n",name);    
    return ENOERR;
                             ...
                             cyg_dir *dir_out )
{
    ...
    else
    {
        ...
        rootfs_node *node = (rootfs_node *)dir;
                             ...
                             struct stat *buf)
{
    ...
    if( err != ENOERR ) return err;
    //if(ds.node == NULL){diag_printf("rootfs_stat fail\n");return ENOERR;}
    // Fill in the status
                             ...
                             int key, void *buf, int len )
{
    ...
    {
    case FS_INFO_CONF:
        err = rootfs_pathconf( ds.node, (struct cyg_pathconf_info *)buf );
        break;
...
static int rootfs_fo_read      (struct CYG_FILE_TAG *fp, struct CYG_UIO_TAG *uio)
{
    rootfs_node *node = (rootfs_node *)fp->f_data;
    int i;
    off_t pos = fp->f_offset;
    ssize_t resid = uio->uio_resid;
    //diag_printf("enter rootfs_fo_read\n");
...
static int rootfs_fo_lseek     (struct CYG_FILE_TAG *fp, off_t *apos, int whence )
{
    rootfs_node *node = (rootfs_node *)fp->f_data;
    off_t pos = *apos;
...
static int rootfs_fo_getinfo   (struct CYG_FILE_TAG *fp, int key, void *buf, int len )
{
    ...
    {
    case FS_INFO_CONF:
        err = rootfs_pathconf( node, (struct cyg_pathconf_info *)buf );
        break;
...
static int rootfs_fo_dirread      (struct CYG_FILE_TAG *fp, struct CYG_UIO_TAG *uio)
{
    rootfs_node *dir = (rootfs_node *)fp->f_data;
    off_t pos = fp->f_offset;
    int err = ENOERR;
    ...
    rootfs_dirent *d = NULL;
    cyg_uint8 *buf;
    size_t size;
    //diag_printf("enter rootfs_fo_dirread\n");   
    ...
    *nbuf = '\0';
    ...
    fp->f_offset = pos+sizeof(rootfs_dirent);
