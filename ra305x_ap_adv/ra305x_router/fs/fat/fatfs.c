...
// -------------------------------------------------------------------------
// Mtab entry.
struct cyg_mtab_entry disk_mte[8] CYG_HAL_TABLE_ENTRY(mtab)={0};
...
fatfs_mount(cyg_fstab_entry *fste, cyg_mtab_entry *mte)
{
    ...
    Cyg_ErrNo           err;
    cyg_uint32 block_size = 512;
    CYG_TRACE2(TFS, "mount fste=%p mte=%p", fste, mte);
	//diag_printf("file test:fsname=%s,devname=%s,dir=%s\n",mte->fsname,mte->devname,mte->name);
    init_fatfs_fds();
    CYG_TRACE1(TFS, "looking up disk device '%s'", mte->devname);

    err = usb_io_lookup(mte->devname, &dev_h);
    if (err != ENOERR)
    	{
        diag_printf("file test:dev not fount\n");return err;}

    disk_channel *tempchan = ((cyg_devtab_entry_t*)dev_h)->priv;
    if((0x0B != tempchan->partition->type) && (0x0C != tempchan->partition->type))
        return ENODEV;
    disk = (fatfs_disk_t *)malloc(sizeof(fatfs_disk_t));
    if (NULL == disk)
        {diag_printf("fatfs_mount malloc error\n");return ENOMEM;}
    ...
    {
        diag_printf("fatfs_mount malloc bcache_mem error\n");
        free(disk);
        return ENOMEM;
    }

    disk->dev_h = (cyg_devtab_entry_t*) malloc (sizeof(cyg_devtab_entry_t));
    if(disk->dev_h == NULL)
    {
        free(disk->bcache_mem);
        free(disk);
    }
    memcpy(disk->dev_h,dev_h,sizeof(cyg_devtab_entry_t));

    disk_channel *chan = (disk_channel *) malloc (sizeof(disk_channel));
    if(chan == NULL)
    {
        free(disk->bcache_mem);
        free((cyg_devtab_entry_t*)(disk->dev_h));
        free(disk);
    }
    memcpy(chan,((cyg_devtab_entry_t*)dev_h)->priv,sizeof(disk_channel));

    //disk_channel *chan = (disk_channel *) ((cyg_devtab_entry_t*)dev_h)->priv;
    block_size = chan->info->block_size;
    ((cyg_devtab_entry_t*)(disk->dev_h))->priv = chan;
    ...
    if (err != ENOERR)
    {
        diag_printf("fatfs_mount fatfs_init error\n");
        //cyg_blib_delete(&disk->blib);
        free(disk->bcache_mem);
        free(((cyg_devtab_entry_t*)(disk->dev_h))->priv);
        free(disk);
        return err;
    }

    err = cyg_blib_io_create(disk->dev_h, disk->bcache_mem,
            CYGNUM_FS_FAT_BLOCK_CACHE_MEMSIZE, disk->cluster_size, &disk->blib);
    if (err != ENOERR)
    {
        diag_printf("fatfs_mount create bcache_mem error\n");
        free(disk->bcache_mem);
        free(((cyg_devtab_entry_t*)(disk->dev_h))->priv);
        free((cyg_devtab_entry_t*)(disk->dev_h));
        free(disk);
...
fatfs_umount(cyg_mtab_entry *mte)
{
    ...
    if (root->refcnt > 1)
       {diag_printf("root->refcnt>1\n"); return EBUSY;}

    /*if (fatfs_get_live_node_count(disk) != 1)
        {diag_printf("live_node_count!=1\n");return EBUSY;}*/
    for(i = 0;i < 10;i++)
    {
        if(fatfs_get_live_node_count(disk) != 1)
            cyg_thread_delay(50);
    ...
    if(fatfs_get_live_node_count(disk) != 1) return EBUSY;
           ...
           cyg_file       *file)
{
	...
	TR(("open mte=%p dir=%p name='%s' mode=%d file=%p\n",
                    mte, dir, name, mode, file));
    ...
    if (err == ENOENT)
    {
        if (ds.last && (mode & O_CREAT))
        {
            ...
            if (NULL == node)
                {TR(("fatfs_node_alloc NULL == node\n"));return EMFILE;}

            // Update directory times
            ds.dir->dentry.atime =
            ds.dir->dentry.mtime = cyg_timestamp();
            TR(("create file succsed\n"));
            err = ENOERR;
        }
    }
    else if (err == ENOERR)
    ...
    if (err != ENOERR)
        {TR(("fatfs_open:err != ENOERR"));return err;}
             ...
             const char     *name2)
{
    ...
    /*diag_printf("rename mte=%p dir1=%p name1='%s' dir2=%p name2='%s'\n",
                    mte, dir1, name1, dir2, name2);*/
              ...
              cyg_file       *file)
{
    ...
    TR(("opendir mte=%p dir=%p name='%s' file=%p\n",
                    mte, dir, name, file));
    init_dirsearch(&ds, disk, (fatfs_node_t *) dir, name);
           ...
           struct stat    *buf)
{
    ...
    TR(("stat mte=%p dir=%p name='%s' buf=%p\n",
                    mte, dir, name, buf));
    init_dirsearch(&ds, disk, (fatfs_node_t *) dir, name);
                 ...
                 const cyg_fs_attrib_t  new_attrib)
{
    ...
    TR(("set_attrib mte=%p dir=%p name='%s' buf=%x\n",
                    mte, dir, name, new_attrib));
...
fatfs_fo_read(struct CYG_FILE_TAG *fp, struct CYG_UIO_TAG *uio)
{
	...
	TR(("fatfs_fo_read fp=%p uio=%p pos=%d\n", fp, uio, pos));
...
fatfs_fo_lseek(struct CYG_FILE_TAG *fp, off_t *apos, int whence)
{
    ...
    TR(("ENTER fatfs_fo_lseek size=%d\n",fd->node->dentry.size));
    switch (whence)
    {
        case SEEK_SET:
...
fatfs_fo_dirread(struct CYG_FILE_TAG *fp, struct CYG_UIO_TAG *uio)
{
    ...
    if (err != ENOERR)
        return (err == EEOF ? ENOERR : err);
    //memset(dentry.filename,0,4*(FM_MAX_FILE_LEN+1));
    memcpy(nbuf, dentry.filename,4*(FM_MAX_FILE_LEN+1));
