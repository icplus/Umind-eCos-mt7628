...
{
	static int txrxCheck = 0;
	UINT32 Value;
	
	MAC_IO_READ32(pAd, ARB_SCR, &Value);

	if( (Value & MT_ARB_SCR_RXDIS) != 0 )
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("rx disable[%x][count=%d]!!! \n",Value,txrxCheck));
		txrxCheck ++;
	}
	else
		txrxCheck = 0;
	...
	if( txrxCheck > 2)
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("Reset rx bit!!! \n"));
		txrxCheck = 0;
