	...
	if ((SetMask & ALLN_SETPROTECT) != 0)
	{
		...
		switch(OperationMode)
		{
			case 0x0:
				...
				if(bNonGFExist)
				{
					MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_INFO, ("%s: GF Protection ON\n", __FUNCTION__));
					Value |= (GF_PROTECTION);
				}
...
VOID MtAsicSetEdcaParm(MP_ADAPTER *pAd, MP_PORT *pPort, PEDCA_PARM pEdcaParm, BOOLEAN bBaseChannel)
{
    ...
    if ((pEdcaParm == NULL) || (pEdcaParm->bValid == FALSE))
    {
        ...
        MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("AsicSetEdcaParm\n"));
...
VOID MtAsicRssiUpdate(MP_ADAPTER *pAd)
{
    ...
    IF_DEV_CONFIG_OPMODE_ON_AP(pAd)
    {
        if ( pAd->MacTab.Size == 0 )
        {
            ...
            if(pEntry == NULL)
            {
                MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("%s(), Line:%d, Mac Entry is NULL\n", __FUNCTION__, __LINE__));
                return;
            }
#else
...
//      RTMP_TX_RATE tx_rate;
    ...
    MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s():WCID=%d, HTPhyMode=0x%x\n",
                __FUNCTION__, pEntry->wcid, pEntry->TxPhyCfg.DoubleWord));

    MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("\tCurTxRateIdx=%d, Mode/BW/MCS/STBC/SGI=%d/%d/%d/%d/%d\n\n",
...
#ifdef DOT11_VHT_AC				
				else if(pEntry->pTable  == RateTableVht1S)
					printk("pTable  == RateTableVht1S\n");
				else if(pEntry->pTable  == RateTableVht1S_MCS9)
					printk("pTable  == RateTableVht1S_MCS9\n");
				else if(pEntry->pTable  == RateTableVht2S)
					printk("pTable  == RateTableVht2S\n");
				else if(pEntry->pTable  == RateTableVht2S_MCS7)
					printk("pTable  == RateTableVht2S_MCS7\n");
				else if(pEntry->pTable  == RateTableVht2S_BW20)
					printk("pTable  == RateTableVht2S_BW20\n");
				else if(pEntry->pTable  == RateTableVht2S_BW40)
					printk("pTable  == RateTableVht2S_BW40\n");
#endif				
				else 
...
VOID MtAsicUpdateBASession(RTMP_ADAPTER *pAd, UCHAR wcid, UCHAR tid, UINT16 sn, UCHAR basize, BOOLEAN isAdd, INT ses_type)
{
	...
	if (ses_type == BA_SESSION_RECP)
	{
		...
		if (isAdd)
		{
            ...
            if(mac_entry == NULL)
            {
                MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("%s(), Line:%d, Mac Entry is NULL  Wcid = %d\n", __FUNCTION__, __LINE__, wcid));
                return;
            }
#else
    ...
    IN  UCHAR TmrType)
{
    ...
    {
        case TMR_INITIATOR:
            MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DbgLvl, ("%s: enable TMR report, as Transmitter\n", __func__));
        ...
        case TMR_RESPONDER:
            MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DbgLvl, ("%s: enable TMR report, as Receiver\n", __func__));
            MtSetTmrCR(pAd, TmrType);
            break;

        case TMR_DISABLE:
        default:
            MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DbgLvl, ("%s: disable TMR report\n", __func__));
            MtSetTmrCR(pAd, TmrType);
            break;
    }
