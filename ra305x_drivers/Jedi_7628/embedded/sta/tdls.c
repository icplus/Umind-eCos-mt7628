	...
	OUT UCHAR	TIIe[])
{
	...
	*pTILen = 0;
	...
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("%s() - Invaild packet length - (action header) \n", __FUNCTION__));
		return FALSE;	
	}
	...
	if (RemainLen < 1)
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("%s() - Invaild packet length - (dialog token) \n", __FUNCTION__));
		return FALSE;
	}	
	*pToken = *Ptr;
	...
	if (RemainLen < 2) 
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("%s() - Invaild packet length - (capability) \n", __FUNCTION__));
		return FALSE;
	}	
	...
	if (LinkIdLen == 0)
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("%s() - Invaild packet - (link identifier) \n", __FUNCTION__));
		return FALSE;
	}
	else
	{
		...
		{
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("%s() - It's not my BSSID\n", __FUNCTION__));
			return FALSE;
		}	
		else if (!MAC_ADDR_EQUAL(LinkIdResponderAddr, pAd->CurrentAddress))
		{
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("%s() - It's not my MAC address\n", __FUNCTION__));
			return FALSE;
		}
	}
	...
	OUT UCHAR	TIIe[])
{
	...
	*pTILen = 0;
	...
	if (RemainLen < 2)
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("%s() - Invaild packet length - (status code) \n", __FUNCTION__));
		return FALSE;
	}	
	...
	OUT UCHAR	FTIe[])
{
	...
	*pFTLen = 0 ;
	...
	if (RemainLen < (LENGTH_802_11 + LENGTH_802_1_H + LENGTH_TDLS_PAYLOAD_H))
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("%s() - Invaild packet length - (cation header) \n", __FUNCTION__));
		return FALSE;	
	}
	...
	if (RemainLen < 2) 
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("%s() - Invaild packet length - (reason code) \n", __FUNCTION__));
		return FALSE;	
	}
	...
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_WARN, ("PeerTdlsTearDownSanity --> Invaild packet length - (link identifier) \n"));
		return FALSE;	
	}
	// It's not my BSSID
	if (!MAC_ADDR_EQUAL(Ptr+2, pAd->CommonCfg.Bssid))
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_WARN, ("PeerTdlsTearDownSanity --> It's not my BSSID\n"));
		return FALSE;
	}	
	...
	else
	{
		...
		if (!MAC_ADDR_EQUAL(pAd->CurrentAddress, LinkIdInitiatorAddr))
		{
			...
			{
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("%s() - It's not my Address\n", __FUNCTION__));
				return FALSE;
			}
			else
			{
	...
	OUT UCHAR	*pToken)
{
	...
	*pToken = *Ptr;
	...
	if ((RemainLen < 20) || (Ptr[0] != IE_TDLS_LINK_IDENTIFIER) || (Ptr[1] != 18))
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("%s() - Invaild packet length - (link identifier) \n", __FUNCTION__));
		return FALSE;
	}
	if (!MAC_ADDR_EQUAL(Ptr+2, pAd->CommonCfg.Bssid))
	{
	...
	OUT USHORT *pChSwitchTimeOut)
{
	...
	*pNewExtChannelOffset = 0;
	...
	if (RemainLen < (LENGTH_802_11 + LENGTH_802_1_H + LENGTH_TDLS_PAYLOAD_H))
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("%s() - Invaild packet length - (ation header) \n", __FUNCTION__));
		return FALSE;	
	}
	...
	if (RemainLen < 1)
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("%s() - Invaild packet length - (target channel) \n", __FUNCTION__));
		return FALSE;
	}	
	...
	*pTargetChannel = *Ptr;
	...
	if (RemainLen < 1)
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("%s() - Invaild packet length - (regulatory class) \n", __FUNCTION__));
		return FALSE;
	}
	
	*pRegulatoryClass = *Ptr;
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s() - Regulatory class = %d \n", __FUNCTION__, *pRegulatoryClass));
	...
	*Peerip = 0;
	...
	if ((P2PIeConLen != 0) && (pP2PIeConLen != NULL))
	{
		...
		*(pP2PIeConLen+1) = (UCHAR)(P2PIeConLen/256);
		...
		{
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_INFO, ("%x-%x-%x-%x-%x-%x-%x-%x-%x-%x-%x-%x-", 
