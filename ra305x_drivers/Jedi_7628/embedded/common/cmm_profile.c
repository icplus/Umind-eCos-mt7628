	...
	IN RTMP_STRING *buffer)
{
	...
	if(RTMPGetKeyParameter("ApCliTrialCh", tmpbuf, 128, buffer, TRUE))
	{
		for (i = 0; i < MAX_APCLI_NUM; i++)
		{
			...
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("TrialChannel=%d\n", pApCliEntry->TrialCh));
		}
	}
        ...
        if(RTMPGetKeyParameter("ApCliBw", tmpbuf, MAX_PARAM_BUFFER_SIZE, buffer, FALSE))
        {
                for (i=0, macptr = rstrtok(tmpbuf,";"); (macptr && i < MAX_APCLI_NUM); macptr = rstrtok(NULL,";"), i++)
                {
                        ...
                        {
                                MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("AUTOBW(%s): unknown ApCliEntry[%d].BW=%d recover to BW_20\n", __FUNCTION__, i, wdev->bw));
                                wdev->bw = BW_20;
                        }
                        MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("AUTOBW(%s): New ApCliEntry[%d].BW=%d \n", __FUNCTION__, i, wdev->bw));
                }
        }
        ...
        if(RTMPGetKeyParameter("ApCliPhyMode", tmpbuf, MAX_PARAM_BUFFER_SIZE, buffer, FALSE))
        {
                ...
                for (i=0, macptr = rstrtok(tmpbuf,";"); (macptr && i < MAX_APCLI_NUM); macptr = rstrtok(NULL,";"), i++)
                {
                        ...
                        {
                                MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("AUTOBW(%s): skip unknown ApCliEntry[%d].WMODE=%d\n", __FUNCTION__, i, wmode));
                                continue;
                        }

                        MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("AUTOBW(%s): New ApCliEntry[%d].WMODE=%d \n", __FUNCTION__, i, wmode));
                }
        }
	...
	IN RTMP_STRING *pBuffer)
{
	...
	do
	{
		...
		/*add by bo.jiao for long&mid sta performence improve*/
		pAd->CommonCfg.LongRssiSet= -88;
		pAd->CommonCfg.MidRssiSet= -60;
		//pAd->CommonCfg.MidRssiCcaH = 3000;
		//pAd->CommonCfg.MidRssiCcaL = 2000;
	...
	*/
...
#ifdef DFS_SUPPORT
	{
		...
		if (RTMPGetKeyParameter("SnifferType", tmpbuf, 10, pBuffer, TRUE)) {
			...
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("SnifferType = %d\n", pAd->sniffer_ctl.sniffer_type
));
		}
      ...
      {
         //long CivicAddrType;
         pAd->pFtmCtrl->Civic.CA_Type = simple_strtol(tmpbuf, 0, 10);
      ...
      if (RTMPGetKeyParameter("LocCivicAddrLength", tmpbuf, 10, pBuffer, TRUE))
      {
         ...
         pAd->pFtmCtrl->Civic.CA_Length = simple_strtol(tmpbuf, 0, 10);
      ...
      {      
      	long LocFloorInfoZ;
      	LocFloorInfoZ = simple_strtol(tmpbuf, 0, 10);
      ...
      if (RTMPGetKeyParameter("LocHeightAboveFloorZ", tmpbuf, 10, pBuffer, TRUE))
      {
		long HeightAboveFloorZ;
      	HeightAboveFloorZ = simple_strtol(tmpbuf, 0, 10);
      ...
      if (RTMPGetKeyParameter("LocHeightAboveFloorUncZ", tmpbuf, 10, pBuffer, TRUE))
      {
		long LocHeightAboveFloorUncZ;
      	LocHeightAboveFloorUncZ = simple_strtol(tmpbuf, 0, 10);
      ...
      {
		long LocReTxAllowed;
      	LocReTxAllowed = simple_strtol(tmpbuf, 0, 10);
      	if(LocReTxAllowed)
      ...
      {
      	long LocRadioMsnt = simple_strtol(tmpbuf, 0, 10);
      ...
      {     
		pAd->pFtmCtrl->FtmNeighbor[0].NeighborPhyType = simple_strtol(tmpbuf, 0, 10);	
      ...
      {     
		pAd->pFtmCtrl->FtmNeighbor[0].NeighborFTMrespCap = simple_strtol(tmpbuf, 0, 10);	
      ...
      {      
		pAd->pFtmCtrl->FtmNeighbor[0].NeighborOpClass = simple_strtol(tmpbuf, 0, 10);	
      ...
      {      	
		pAd->pFtmCtrl->FtmNeighbor[0].NeighborChannel = simple_strtol(tmpbuf, 0, 10);	
	...
	IN CHAR				base_pwr)
{
	...
	for ( i = 0 ; i < 5; i++ )
	{
		...
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("20MHz BW, 2.4G band-%08x,  Adata = %08x,  Gdata = %08x \n", data, Adata, Gdata));
	}
