								...
								INT32 pkt_type)
{
    ...
    if (pHeader && (pHeader->FC.SubType== SUBTYPE_AUTH))
    {
        /*
                If HW already decrypted this packet, SW doesn't need to decrypt again.
                @20150708
        */
        if ((rx_base->rxd_2.sec_mode != 0) &&
            (rx_base->rxd_2.cm == 0) &&
            (rx_base->rxd_2.clm == 0))
        {
        ...
        MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,
			...
			rx_base->rxd_2.sec_mode, rx_base->rxd_2.cm, rx_base->rxd_2.clm, rx_base->rxd_2.tkip_mic_err, rx_base->rxd_2.icv_err));
    }
...
UINT32 parse_rx_packet_type(RTMP_ADAPTER *ad, RX_BLK *rx_blk, VOID *rx_packet)
{
	...
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("drop rx length = 0x%x, pdma length = 0x%x packet\n", 
	...
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("drop rx length = 0x%x packet\n", 
...
#if 0//def __ECOS /* keep the management ring has more space */
            ...
            (pHeader_802_11->FC.SubType == SUBTYPE_ACTION))
    {
        ...
        if (IsPublicActionFrame(pAd, pHeader_802_11))
        {
            ...
            if (pFtmEntry && pFtmEntry->bNeedTmr)
            {
                ...
                RTMPSetTimer(&pFtmEntry->FtmTxTimer, 50);
        ...
        */
        ...
        IF_DEV_CONFIG_OPMODE_ON_STA(pAd)
        {
            ...
            if((pHeader_802_11->FC.SubType == SUBTYPE_DATA_NULL) || 
               (pHeader_802_11->FC.SubType == SUBTYPE_QOS_NULL))
            {
                PID = (pHeader_802_11->FC.PwrMgmt == PWR_ACTIVE) ? PID_NULL_FRAME_PWR_ACTIVE : PID_NULL_FRAME_PWR_SAVE;
    			if (!RTMP_TEST_FLAG(pAd, fRTMP_ADAPTER_BSS_SCAN_IN_PROGRESS) && RTMPEnterPsmNullBitStatus(&pAd->StaCfg.PwrMgmt))
    			{
    				...
    				TxSTypeCtl(pAd, PID, TXS_FORMAT0, FALSE, TRUE); 
    			}
    			else
    			{
    				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s(line=%d)\n", __FUNCTION__, __LINE__));
    				TxSTypeCtl(pAd, PID, TXS_FORMAT0, FALSE, FALSE); 
    			}
            }
        }
#endif /* CONFIG_STA_SUPPORT */
...
//---Add by shiang for debug
    ...
    {
        MtSetTmrCR(pAd, TMR_INITIATOR);
        pAd->TmrState = SEND_OUT;
    }
#endif /* TMR_SUPPORT */
...
#error
		...
		if (RTMP_GET_PACKET_MOREDATA(pPacket) || (pMacEntry->PsMode == PWR_SAVE))
			TxFrameType = TX_LEGACY_FRAME;
		else if (!OPSTATUS_TEST_FLAG(pAd, fOP_STATUS_WMM_INUSED) || !CLIENT_STATUS_TEST_FLAG(pMacEntry, fCLIENT_STATUS_WMM_CAPABLE))
			TxFrameType = TX_LEGACY_FRAME;				
		else
#ifdef UAPSD_SUPPORT
...
VOID rtmp_ps_listforAll(RTMP_ADAPTER *pAd)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_LOUD, ("List:\t"));
	DlListForEach(pEntry,&pAd->psTokenQueue,struct WCID_TABLE,list)
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_LOUD, ("%d\t",pEntry->wcid));
	}
...
INT rtmp_ps_enq(RTMP_ADAPTER *pAd, STA_TR_ENTRY *tr_entry)
{
	...
	if( tr_entry->PsTokenFlag != PS_TOKEN_STAT_WCID_PKT)
	{
		...
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_LOUD, ("enqueue PS fifo token to Token Queue, WCID: %d,%d.\n", tr_entry->wcid,pEntry->wcid));
		
	}else
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_LOUD, ("not need enqueue WCID: %d.\n", tr_entry->wcid));
...
INT rtmp_psDeq_req(RTMP_ADAPTER *pAd)
{
	...
	DlListForEachSafe(psEntry,tmp,&pAd->psTokenQueue,struct WCID_TABLE,list){
		...
		{
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_LOUD, ("%s(): wcid >MAX_LEN_OF_TR_TABLE \n",__FUNCTION__));
		...
		if(!tr_entry)
		{
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_LOUD, ("%s(): tr_entry is NULL \n",__FUNCTION__));
			continue;
		}
		...
		{	
			...
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_LOUD, ("%s(): ps token flag =%d,wcid =%d\n",__FUNCTION__,tr_entry->PsTokenFlag,psEntry->wcid));
			continue;
		}		
		...
		for(i=0;i<cnt;i++)
		{
			...
			{			
				...
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_LOUD, ("%s(): fifo is full,flag:%d,enqId:%d,deqId:%d, cap: %d \n",__FUNCTION__,tr_entry->PsTokenFlag,fifo_swq->enqIdx,fifo_swq->deqIdx,tr_entry->enq_cap));
				break;
			}
		...
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_LOUD, ("%s(): deq  for PS retrieve and PSM change. WCID: %d,stat: %d\n",__FUNCTION__,tr_entry->wcid,tr_entry->PsTokenFlag));
		return TRUE;
	}
...
INT rtmp_psDeq_report(RTMP_ADAPTER *pAd,struct dequeue_info *info)
{
	...
	{
		rtmp_ps_enq(pAd,tr_entry);
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_LOUD, ("%s(): After DeqReport, re-enq PsToken, WCID: %d,STAT: %d\n",__FUNCTION__,info->target_wcid,tr_entry->PsTokenFlag));
	}else
	if(tr_entry->enqCount <= 0 && tr_entry->PsTokenFlag != PS_TOKEN_STAT_IDLE)
	{		
		tr_entry->PsTokenFlag = PS_TOKEN_STAT_IDLE;		
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_LOUD, ("%s(): After DeqReport, Abort PsToken, WCID: %d,STAT: %d\n",__FUNCTION__,info->target_wcid,tr_entry->PsTokenFlag));
	}
	return TRUE;
}
...
INT rtmp_enq_req(RTMP_ADAPTER *pAd, PNDIS_PACKET pkt, UCHAR qidx, STA_TR_ENTRY *tr_entry, BOOLEAN FlgIsLocked,QUEUE_HEADER *pPktQueue)
{
	...
	enq_idx = fifo_swq->enqIdx;
		...
		occupied_wcid = fifo_swq->swq[enq_idx];
#ifdef TRACELOG_IPARP
	TRACE_IPARP_TX_CHK_PNT(pAd, RTMP_GET_PACKET_TRACEIPARP(pkt), P3, 0);
		if (RTMP_GET_PACKET_TRACEIPARP(pkt) != INVALID_INDEX)
			printk("\033[1;33m %s, %u, occupied_wcid = %u, enq_cap = %u\033[0m\n", __FUNCTION__, __LINE__, occupied_wcid, tr_entry->enq_cap);  /* Haipin Debug Print (Y)*/
#endif /* TRACELOG_IPARP */
		enq_done = FALSE;
#ifdef DATA_QUEUE_RESERVE
			if (RTMP_GET_PACKET_DHCP(pkt) || RTMP_GET_PACKET_EAPOL(pkt)
...
#endif /* DBG_DIAGNOSE */
#ifdef TRACELOG_IPARP
		TRACE_IPARP_TX_CHK_PNT(pAd, RTMP_GET_PACKET_TRACEIPARP(pkt), P11, 0);
#endif /* TRACELOG_IPARP */
...
INT TxOPUpdatingAlgo(RTMP_ADAPTER *pAd)
{
	...
	/* It means make sure TXOP 0x80 only for one apclient to RootAp and no sta connected in our Extender,
		...
		{
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Hit the traffic rule with cnt ApClient[%d],STA[%d]\n", 
