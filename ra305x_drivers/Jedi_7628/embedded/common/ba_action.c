...
/*		 (pBAEntry->list.qlen > (pBAEntry->BAWinSize/8)))*/
		if (RTMP_TIME_AFTER((unsigned long)Now32, (unsigned long)(pBAEntry->LastIndSeqAtTimer+(MAX_REORDERING_PACKET_TIMEOUT/15))) /*add for BA Drop issue,500ms -> 200ms, 6->15*/
...
#endif
	{
		...
		*/
		...
		if (ba_reordering_mpdu_insertsorted(&pBAEntry->list, mpdu_blk) == FALSE)
		{
			...
			TRACE_IPARP_RX_CHK_PNT(pAd, RTMP_GET_PACKET_TRACEIPARP(pRxBlk->pRxPacket), P10, 0);
...
VOID Indicate_AMPDU_Packet(RTMP_ADAPTER *pAd, RX_BLK *pRxBlk, UCHAR wdev_idx)
{
	...
	else if (SEQ_SMALLER(Sequence, pBAEntry->LastIndSeq, MAXSEQ))
	{
		...
		pBAEntry->nDropPacket++;
#ifdef TRACELOG_IPARP
		TRACE_IPARP_RX_CHK_PNT(pAd, RTMP_GET_PACKET_TRACEIPARP(pRxBlk->pRxPacket), P9, 0);
#endif /* TRACELOG_IPARP */
		if (pRxBlk->Ping)
		{
			INDICATE_LEGACY_OR_AMSDU(pAd, pRxBlk, wdev_idx);
...
/*add for BA Drop issue*/
VOID BARecForceFlushAllEntry(RTMP_ADAPTER *pAd, UCHAR Flag)
{
  UINT32 MacEntryIdx = 0;
  UINT32 TidIdx = 0;
  USHORT Idx = 0;
  ULONG Now32 = 0;
  ...
  for (MacEntryIdx = 0; MacEntryIdx < MAX_LEN_OF_MAC_TABLE; MacEntryIdx++) 
  	{
     	pMacEntry = &pAd->MacTab.Content[MacEntryIdx];
      	if (IS_ENTRY_NONE(pMacEntry))
       		continue;

	  	for (TidIdx = 0; TidIdx < NUM_OF_TID; TidIdx++) {
			
	        Idx = pMacEntry->BARecWcidArray[TidIdx];
	        if (Idx == 0)
	          continue;
			...
			if( Flag )
			{//flush all
				if (pBAEntry->list.qlen > 1) {
					ba_refresh_reordering_mpdus(pAd, pBAEntry);
					pBAEntry->LastIndSeqAtTimer = Now32;
					MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("BA_FORCE_FLUSH_ALL: MacEntry(%d):TID(%d):LastIndSeq(%d):RecSeq(%d)\n",
						MacEntryIdx,TidIdx,pBAEntry->LastIndSeq,pBAEntry->rcvSeq));
				}
			}
			else 
			{//Update Time to Now32 and check 200ms is arrived
				if (RTMP_TIME_AFTER((unsigned long)Now32, (unsigned long)(pBAEntry->LastIndSeqAtTimer+(MAX_REORDERING_PACKET_TIMEOUT/15))) 
				&&(pBAEntry->list.qlen > 0)){
					ba_refresh_reordering_mpdus(pAd, pBAEntry);
					pBAEntry->LastIndSeqAtTimer = Now32;
					MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("timeout[%d] (%08lx-%08lx = %d > %d): %x, flush all!\n ", pBAEntry->list.qlen, Now32, (pBAEntry->LastIndSeqAtTimer), 
