...
SanityCheck:
	...
	if ((Sanity != 0x7) || ( bWscCheck == FALSE))
	{
		...
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("%s Sanity Failed\n", __FUNCTION__));
#endif /* WIDI_SUPPORT */
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_LOUD, ("%s() - missing field, Sanity=0x%02x, bWscCheck=%d\n", __FUNCTION__, Sanity,bWscCheck));
		return FALSE;
	}
	}
	else
        ...
        switch(eid)
        {
			...
			case IE_HT_CAP:
				if (eid_len >= SIZE_HT_CAP_IE)
					...
					MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_WARN, ("%s() - wrong IE_HT_CAP. eid_len = %d\n", __FUNCTION__, eid_len));
