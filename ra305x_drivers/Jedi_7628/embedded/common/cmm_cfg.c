...
INT RT_CfgSetWirelessMode(RTMP_ADAPTER *pAd, RTMP_STRING *arg)
{
	...
	if (pChipCap->ac_off_mode && WMODE_CAP_AC(wmode)) {
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("it doesn't support VHT AC!\n"));
		wmode &= ~(WMODE_AC);
	}
#endif /* MT76x2 */
#endif /* DOT11_VHT_AC */
...
INT SetSCSCfg_Proc(RTMP_ADAPTER *pAd, RTMP_STRING *arg)
{
	...
	if (Recv != 6){
		...
		MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("iwpriv ra0 set SCSCfg=[MinRssiTolerance]-[TrafficThreshold]-[FalseCcaUpBoundary]-[FalseCcaLowBoundary]-[FixedBoundary]-[ForceMode]\n"));
		MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("PS: FiexedBond is Negative number. Ex:70 means -70dBm"));
	} else {
...
INT SetRffOFF_Proc(RTMP_ADAPTER *pAd, RTMP_STRING *arg)
{
	UINT32 val = 0;
	UINT32 mac_val = 0;
	val = simple_strtol(arg, 0, 10);
	...
	if (val == TRUE)
	{
				...
				RTMP_IO_READ32(pAd, 0x10d00, &mac_val);
				mac_val &= ~( BIT31 );
				RTMP_IO_WRITE32(pAd, 0x10d00, mac_val);
				
				RTMP_IO_READ32(pAd, 0x10d14, &mac_val);
				mac_val |= ( BIT12 );
				RTMP_IO_WRITE32(pAd, 0x10d14, mac_val);	
				
				RTMP_IO_READ32(pAd, 0x11d00, &mac_val);
				mac_val &= ~( BIT31 );
				RTMP_IO_WRITE32(pAd, 0x11d00, mac_val);
				
				RTMP_IO_READ32(pAd, 0x11d14, &mac_val);
				mac_val |= ( BIT12 );
				RTMP_IO_WRITE32(pAd, 0x11d14, mac_val);	

				RTMP_IO_READ32(pAd, 0x10200, &mac_val);	
				mac_val |= ( BIT1);
				mac_val &= ~( BIT0);
				mac_val |= ( BIT5);
				mac_val &= ~( BIT4);
				mac_val |= ( BIT15);
				mac_val &= ~( BIT14);
				RTMP_IO_WRITE32(pAd, 0x10200, mac_val);		

				RTMP_IO_READ32(pAd, 0x11200, &mac_val);	
				mac_val |= ( BIT1);
				mac_val &= ~( BIT0);
				mac_val |= ( BIT5);
				mac_val &= ~( BIT4);
				mac_val |= ( BIT15);
				mac_val &= ~( BIT14);
				RTMP_IO_WRITE32(pAd, 0x11200, mac_val);		
	}else {
		RTMP_IO_READ32(pAd, 0x10200, &mac_val);	
		mac_val &= ~ ( BIT0|BIT1|BIT5|BIT4|BIT14|BIT15);
		RTMP_IO_WRITE32(pAd, 0x10200, mac_val);
