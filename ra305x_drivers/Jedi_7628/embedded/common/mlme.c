	...
	IN PRTMP_ADAPTER pAd)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s::pAd->StaCfg.WindowsBatteryPowerMode(%lu)\n", __FUNCTION__, pAd->StaCfg.WindowsBatteryPowerMode));
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s::pAd->StaCfg.WindowsPowerMode(%lu)\n", __FUNCTION__, pAd->StaCfg.WindowsPowerMode));
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s::pAd->CommonCfg.bAPSDForcePowerSave(%d)\n", __FUNCTION__, pAd->CommonCfg.bAPSDForcePowerSave));
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s::fOP_STATUS_DOZE(%d)\n", __FUNCTION__, OPSTATUS_TEST_FLAG(pAd, fOP_STATUS_DOZE)));
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s::fpAd->StaCfg.PwrMgmt.psm(%d)\n", __FUNCTION__, pAd->StaCfg.PwrMgmt.Psm));
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("*****************************************************\n\n"));
}
...
VOID RTMPOffloadPm(RTMP_ADAPTER *pAd, UINT8 ucWlanIdx, UINT8 ucPmNumber, UINT8 ucPmState)
{
	...
	if (!OPSTATUS_TEST_FLAG(pAd, fOP_STATUS_DOZE) && (ucPmState == ENTER_PM_STATE))
	{
        ...
        /* Move CmdExtPwrMgtBitWifi() to MlmeCheckPsmChange() */
	...
	else if (OPSTATUS_TEST_FLAG(pAd, fOP_STATUS_DOZE) && (ucPmState == EXIT_PM_STATE))
	{
        ...
        /* In Legacy PSP, always keep PM bit = 1 thought PM bit is controlled by H/W */
...
VOID MlmeHandler(RTMP_ADAPTER *pAd) 
{
		...
		) 
	{
		...
		if (Elem)
		{
			...
			{
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("!!! reset MLME state machine !!!\n"));
...
NDIS_STATUS MlmeInit(RTMP_ADAPTER *pAd)
{
...

	{
		...
		if (Status == NDIS_STATUS_FAILURE) {
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR,  ("%s: unable to start MlmeThread\n", RTMP_OS_NETDEV_GET_DEVNAME(pAd->net_dev)));
		}
	}
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("<-- MLME Initialize\n"));
...
//#if defined(RTMP_MAC_USB) || defined(RTMP_SDIO_SUPPORT)
	...
	*/
	if (pAd->CountDowntoPsm > 0)
		pAd->CountDowntoPsm--;
#endif /* defined(RTMP_MAC_USB) || defined(RTMP_SDIO_SUPPORT) */
...
/*	RECBATimerTimeout(SystemSpecific1,FunctionContext,SystemSpecific2,SystemSpecific3);*/
	...
	/*add for BA Drop issue*/
	BARecForceFlushAllEntry(pAd, 0);
	...
	*/
		...
		RTMP_IO_READ32 (pAd, WF_PHY_BASE+0x410c, &PDCnt);	
		RTMP_IO_READ32 (pAd, WF_PHY_BASE+0x4120, &MDRDYCnt);		

		RTMP_IO_WRITE32 (pAd,  WF_PHY_BASE+0x4108, 0xc0);
		RTMP_IO_WRITE32 (pAd,  WF_PHY_BASE+0x4108, 0x80);
		
		pAd->RalinkCounters.FalseCCACnt =(((PDCnt & 0x0000ffff) - (MDRDYCnt & 0x0000ffff)) 
			+ ((PDCnt >> 16 & 0x0000ffff) - (MDRDYCnt>> 16 & 0x0000ffff)));
	...
	{
#if 0
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s, %u, NumOfChannels = %u, LastScanChannel = %u, bScanning = %u, BreakTime = %u\n",
				__FUNCTION__, __LINE__,
	...
	*/
		...
		&& (NdisEqualMemory(pAd->CurrentAddress, pAd->EthConvert.EthCloneMac, MAC_ADDR_LEN) == FALSE))
	{
		...
		else
		{
			...
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("Write EthCloneMac to ASIC: =%02x:%02x:%02x:%02x:%02x:%02x\n",
			PRINT_MAC(pAd->CurrentAddress)));

			if(pAd->EthConvert.SSIDStrLen != 0)
			{
				/*MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("copy MlmeAux.Ssid to AutoReconnect!\n"));*/
		...
		if ((!RTMP_TEST_FLAG(pAd, fRTMP_ADAPTER_BSS_SCAN_IN_PROGRESS)))
		{
			if (pAd->StaCfg.wdev.UapsdInfo.bAPSDCapable && pAd->CommonCfg.APEdcaParm.bAPSDCapable)
			{
				...
				if ((pAd->Mlme.OneSecPeriodicRound % 20) == 8)
				{
                    ...
                    if (OPSTATUS_TEST_FLAG(pAd, fOP_STATUS_DOZE))
					{
						MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_INFO, ("%s::(line:%d)\n", __FUNCTION__, __LINE__));
...
/*
	...
	Output:
		1. change pAd->StaCfg.PwrMgmt.Psm to PWR_SAVE or leave it untouched
...
#ifdef RELEASE_EXCLUDE
			/* Can't add debug print here. It'll make Mlme Stat machine freeze.*/
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_INFO, ("%s(): LINE:%d, full and dropped\n", __FUNCTION__, __LINE__));
#endif /* RELEASE_EXCLUDE */
			RTMP_MLME_HANDLER(pAd);
			return FALSE;
		}
