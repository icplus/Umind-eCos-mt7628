...
VOID PeerPublicAction(RTMP_ADAPTER *pAd, MLME_QUEUE_ELEM *Elem) 
{
	...
	if ((Elem->OpMode == OPMODE_AP) || (pAd->OpMode == OPMODE_AP))
...
#endif /* P2P_SUPPORT */
				{
					...
					{
						MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("BSS_2040_COEXIST: BW=%d, bBssCoexEnable=%d!\n", pAd->CommonCfg.RegTransmitSetting.field.BW, pAd->CommonCfg.bBssCoexEnable));
					}
...
VOID SendRefreshBAR(RTMP_ADAPTER *pAd, MAC_TABLE_ENTRY *pEntry) 
{
	...
	for (i = 0; i <NUM_OF_TID; i++)
	{
		...
		if  (pBAEntry->ORI_BA_Status == Originator_Done)
		{
			...
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_INFO, ("BA --> NoBADataCountDown  [= %d]!!!\n",
