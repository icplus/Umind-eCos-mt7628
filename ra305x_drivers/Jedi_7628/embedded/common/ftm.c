	...
	)
{
	...
	UCHAR VHT_BW = pAd->CommonCfg.vht_bw;
	UCHAR HT_BW = pAd->CommonCfg.RegTransmitSetting.field.BW;
...
{
	return (pAd->pFtmCtrl->DialogToken & MASK_PID_FTM) + NUMBER_PID_FTM + 1;
}
	...
	)
{
	...
	NdisZeroMemory(&pEntry->PidList, sizeof(MT_DL_LIST));
	pEntry->PendingPid = FTMPID_NOT_WAITING;
	...
	)
{
	...
	pTmr->HwReport.tmr_d6.field.tod_32 = 0xFFFF;
	pTmr->HwReport.tmr_d6.field.toa_32 = 0xFFFF;
	pTmr->HwReport.tod_0 = 0xFFFFFFFF;
	pTmr->HwReport.toa_0 = 0xFFFFFFFF;
}
	...
	)
{
	...
	if (pTmr->tmr_d6.field.tod_32 > pTmr->tmr_d6.field.toa_32)
	{
...
void FtmMapSigmaCmdToLocLCI(RTMP_ADAPTER *pAd,RTMP_STRING *tmpbuf ,PLCI_FIELD pLci)
{
	...
	*/
	...
	for(i=0;i<46;i++)
	{
		printk("\033[1;33m%c\033[0m",tmpbuf[i]);  /* Kyle Debug Print */
	}
	printk("\n");
...
VOID FtmProfileNeighborApParse(RTMP_ADAPTER *pAd,UINT NeighborIdx, RTMP_STRING *tmpbuf)
{
    ...
    {
        MTWF_LOG(DBG_CAT_PROTO, CATPROTO_FTM, DBG_LVL_ERROR,("\033[1;34m %s lalala NeighborInfo  %s \033[0m\n", __FUNCTION__, NeighborInfo));   
        ...
        else
        {
            ...
            {						
            	...
            	pAd->pFtmCtrl->FtmNeighbor[NeighborIdx].NeighborPhyType = (UINT8)simple_strtol(pch+1, 0, 10);
            	MTWF_LOG(DBG_CAT_PROTO, CATPROTO_FTM, DBG_LVL_ERROR,("\033[1;32m %s, NeighborPhyType  %d \033[0m\n", __FUNCTION__, pAd->pFtmCtrl->FtmNeighbor[NeighborIdx].NeighborPhyType));   
            }
            else if((strstr(NeighborInfo, "FTMinBssidInfo") != 0))
            {						
            	pch = strchr(NeighborInfo,':');
            	pAd->pFtmCtrl->FtmNeighbor[NeighborIdx].NeighborFTMCap = (UINT8)simple_strtol(pch+1, 0, 10);
            	MTWF_LOG(DBG_CAT_PROTO, CATPROTO_FTM, DBG_LVL_ERROR,("\033[1;32m %s, NeighborFTMCap  %d \033[0m\n", __FUNCTION__, pAd->pFtmCtrl->FtmNeighbor[NeighborIdx].NeighborFTMCap));   
            }
            else if((strstr(NeighborInfo, "OpChannel") != 0))
            {
            	pch = strchr(NeighborInfo,':');						
            	pAd->pFtmCtrl->FtmNeighbor[NeighborIdx].NeighborChannel = (UINT8)simple_strtol(pch+1, 0, 10);
            	MTWF_LOG(DBG_CAT_PROTO, CATPROTO_FTM, DBG_LVL_ERROR,("\033[1;32m %s, NeighborChannel  %d \033[0m\n", __FUNCTION__, pAd->pFtmCtrl->FtmNeighbor[NeighborIdx].NeighborChannel));   
            }
            else if((strncmp(NeighborInfo, "LocCivicAddr:",13) == 0))
            {
            	...
            	MTWF_LOG(DBG_CAT_PROTO, CATPROTO_FTM, DBG_LVL_ERROR,("\033[1;32m %s, LocCivicAddr  %s \033[0m\n", __FUNCTION__, pAd->pFtmCtrl->FtmNeighbor[NeighborIdx].CA_Value));   
            }
            else if((strstr(NeighborInfo, "LocCivicAddrType") != 0))
            {
            	pch = strchr(NeighborInfo,':');						
            	pAd->pFtmCtrl->FtmNeighbor[NeighborIdx].Civic.CA_Type = (UINT8)simple_strtol(pch+1, 0, 10);
            	MTWF_LOG(DBG_CAT_PROTO, CATPROTO_FTM, DBG_LVL_ERROR,("\033[1;32m %s, LocCivicAddrType  %d \033[0m\n", __FUNCTION__, pAd->pFtmCtrl->FtmNeighbor[NeighborIdx].Civic.CA_Type));   
            }
            else if((strstr(NeighborInfo, "LocCivicAddrLength") != 0))
            {
            	pch = strchr(NeighborInfo,':');						
            	pAd->pFtmCtrl->FtmNeighbor[NeighborIdx].Civic.CA_Length = (UINT8)simple_strtol(pch+1, 0, 10);
    ...
    , __FUNCTION__, NeighborIdx, PRINT_MAC(pAd->pFtmCtrl->FtmNeighbor[NeighborIdx].NeighborBSSID)));
...
{
	UINT16 value = simple_strtol(arg, 0, 10);
	MTWF_LOG(DBG_CAT_PROTO, CATPROTO_FTM, DBG_LVL_OFF, 
		("%s(): RandomizationInterval %d\n", __FUNCTION__, pAd->pFtmCtrl->RandomizationInterval));
	...
	)
{
	UINT8 value = simple_strtol(arg, 0, 10);
	MTWF_LOG(DBG_CAT_PROTO, CATPROTO_FTM, DBG_LVL_OFF, 
		("%s(): MinimumApCount %d\n", __FUNCTION__, pAd->pFtmCtrl->MinimumApCount));
	...
	IN RTMP_STRING *arg)
{
	...
	UINT8 DialogToken = RandomByte(pAd);
	UINT8 MeasurementToken = RandomByte(pAd);
	UCHAR PeerMACAddr[MAC_ADDR_LEN]={0};
	RTMP_STRING *value;
	UINT  i=0;
	PFTM_CTRL	pFtm = pAd->pFtmCtrl;
	BSS_STRUCT *pMbss;
	RRM_BSSID_INFO BssidInfo;
	UINT8 CondensedPhyType = (pAd->CommonCfg.Channel > 14)?9:7; //7:2G,9:5G
	//PRRM_CONFIG pRrmCfg;
	...
	if(strlen(arg) == 17)  /* Mac address acceptable format 01:02:03:04:05:06 length 17 */
	{
		for (i=0, value = rstrtok(arg,":"); value; value = rstrtok(NULL,":"), i++)
		{
			if((strlen(value) != 2) || (!isxdigit(*value)) || (!isxdigit(*(value+1))) )
			{
				MTWF_LOG(DBG_CAT_PROTO, CATPROTO_FTM, DBG_LVL_OFF, ("%s() MAC Invalid. \n", __FUNCTION__));
				return FALSE;  /* Invalid */
			}

			AtoH(value, &PeerMACAddr[i], 1);
		}

		if(i != 6)
		{
			MTWF_LOG(DBG_CAT_PROTO, CATPROTO_FTM, DBG_LVL_OFF, ("%s() MAC len != 6 ,Invalid. \n", __FUNCTION__));
			return FALSE;  /* Invalid */
		}
	}
	...
	if(pEntry == NULL)
	{
		MTWF_LOG(DBG_CAT_PROTO, CATPROTO_FTM, DBG_LVL_OFF, ("%s() can't find entry for this MAC. \n", __FUNCTION__));
		return FALSE;  /* Invalid */
	}
	else if(!pEntry->RrmEnCap.field.FTMRangeReportCapability)
	{
		MTWF_LOG(DBG_CAT_PROTO, CATPROTO_FTM, DBG_LVL_OFF, ("%s() this STA's FTMRangeReportCap is not enabled return. \n", __FUNCTION__));
		return FALSE;  
	}
#if 0

	pFtmEntry = FtmEntrySearch(pAd, PeerMACAddr);
	if(pFtmEntry == NULL)
	{
		MTWF_LOG(DBG_CAT_PROTO, CATPROTO_FTM, DBG_LVL_OFF, ("%s() can't find FtmEntry for this MAC. \n", __FUNCTION__));
		return FALSE;  /* Invalid */
	}
#endif
	...
	/* build action frame header. */
	MgtMacHeaderInit(pAd, &ActHdr, SUBTYPE_ACTION, 0, PeerMACAddr,pMbss->wdev.if_addr, pMbss->wdev.if_addr);

	NdisMoveMemory(pOutBuffer, (PCHAR)&ActHdr, sizeof(HEADER_802_11));
	FrameLen = sizeof(HEADER_802_11);

	InsertActField(pAd, (pOutBuffer + FrameLen), &FrameLen,
			CATEGORY_RM, RRM_MEASURE_REQ);
	/* fill Dialog Token */
	InsertDialogToken(pAd, (pOutBuffer + FrameLen), &FrameLen, DialogToken);
