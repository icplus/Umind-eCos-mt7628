...
#ifdef BANDWIDTH_10_SUPPORT
INT	Set_Bw5Bw10_Proc(
	IN	PRTMP_ADAPTER	pAd,
	IN	RTMP_STRING *arg)
{
	BOOLEAN	bBw5Bw10;

	bBw5Bw10 = simple_strtol(arg, 0, 10); /*1=BW10 2=BW5*/
	pAd->CommonCfg.Bw5Bw10= bBw5Bw10;
	Set_Cmm_WirelessMode_Proc(pAd, "4", 0);
#ifdef CONFIG_AP_SUPPORT
		IF_DEV_CONFIG_OPMODE_ON_AP(pAd)
		{	
			APStop(pAd);
...
/* It is required to update Commoncfg.channel with new valid channel if user has entered wrong channel value.*/
	pAd->CommonCfg.Channel = Channel;
#ifdef CONFIG_AP_SUPPORT
	IF_DEV_CONFIG_OPMODE_ON_AP(pAd)
	{
...
/*add by bo.jiao for long&Mid Sta Performence improve 20160217*/
INT show_RssiSet_proc(RTMP_ADAPTER *pAd, RTMP_STRING *arg)
{
	printk("show_RssiSet_proc:==========>\n");
	printk(" LongRssiSet : %d\n", pAd->CommonCfg.LongRssiSet);
	printk(" MidRssiSet  : %d\n", pAd->CommonCfg.MidRssiSet);
	//printk(" MidRssiCcaH : %d\n", pAd->CommonCfg.MidRssiCcaH);
	//printk(" MidRssiCcaL : %d\n", pAd->CommonCfg.MidRssiCcaL);
	printk("show_RssiSet_proc:<==========\n");
}
...
INT show_trinfo_proc(RTMP_ADAPTER *pAd, RTMP_STRING *arg)
{
	...
	if (IS_RBUS_INF(pAd) || IS_PCI_INF(pAd))
	{
		...
		UINT32 tcidx[NUM_OF_TX_RING], tdidx[NUM_OF_TX_RING], tfidx[NUM_OF_TX_RING];
		UINT32 rbase[NUM_OF_RX_RING], rcnt[NUM_OF_RX_RING];
		UINT32 rcidx[NUM_OF_RX_RING], rdidx[NUM_OF_RX_RING], rfidx[NUM_OF_RX_RING];;
		UINT32 mbase[4] = {0}, mcnt[4] = {0}, mcidx[4] = {0}, mdidx[4] = {0}, mfidx[4] = {0};
		UINT32 sys_ctrl[4];
		...
		for (idx = 0; idx < TxHwRingNum; idx++)
		{
			...
			tfidx[idx] = pAd->TxRing[idx].TxSwFreeIdx;
		}
		...
		for (idx = 0; idx < NUM_OF_RX_RING; idx++)
		{
			...
			rfidx[idx] = pAd->RxRing[idx].RxSwReadIdx;
		}
		...
		mfidx[0]= pAd->MgmtRing.TxSwFreeIdx;
			...
			mfidx[1]= pAd->CtrlRing.TxSwFreeIdx;
#endif /* CONFIG_ANDES_SUPPORT */
		...
		if (pAd->chipCap.hif_type == HIF_MT)
		{
			...
			mfidx[2]= pAd->BcnRing.TxSwFreeIdx;
			...
			mfidx[3]= pAd->TxBmcRing.TxSwFreeIdx;
		}
#endif /* MT_MAC */
	...
	// TODO: shiang-7603
		...
		/*add for TxRx Coherent record */
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("TxRx Coherent Counter\n"));
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("\tTxCoherentCnt: %d\n",pAd->TxCoherentCnt));
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("\tRxCoherentCnt: %d\n",pAd->RxCoherentCnt));
		...
		for (idx = 0; idx < NUM_OF_TX_RING; idx++) {
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("\t%d \t0x%04x \t0x%08x \t0x%x \t0x%x \t0x%x \t0x%x\n",
						idx, pAd->TxRing[idx].hw_desc_base, tbase[idx], tcnt[idx], tcidx[idx], tdidx[idx], tfidx[idx]));
		}
		...
		for (idx = 0; idx < NUM_OF_RX_RING; idx++) {
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("\t%d \t0x%04x \t0x%08x \t0x%x \t0x%x \t0x%x \t0x%x\n",
						idx, pAd->RxRing[idx].hw_desc_base, rbase[idx], rcnt[idx], rcidx[idx], rdidx[idx], rdidx[idx]));
		}

		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("\nMgmtRing Configuration\n"));
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("\tRingIdx Reg \tBase \t\tCnt \tCIDX \tDIDX \tFIDX\n"));
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("\t%d \t0x%04x \t0x%08x \t0x%x \t0x%x \t0x%x  \t0x%x\n",
									0, pAd->MgmtRing.hw_desc_base, mbase[0], mcnt[0], mcidx[0], mdidx[0], mfidx[0]));

#ifdef CONFIG_ANDES_SUPPORT
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("\nCtrlRing Configuration\n"));
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("\tRingIdx Reg \tBase \t\tCnt \tCIDX \tDIDX \tFIDX\n"));
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("\t%d \t0x%04x \t0x%08x \t0x%x \t0x%x \t0x%x  \t0x%x\n",
									0, pAd->CtrlRing.hw_desc_base, mbase[1], mcnt[1], mcidx[1], mdidx[1], mfidx[1]));
#endif /* CONFIG_ANDES_SUPPORT */
		...
		if (pAd->chipCap.hif_type == HIF_MT)
		{
			...
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("\tRingIdx Reg \tBase \t\tCnt \tCIDX \tDIDX \tFIDX\n"));
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("\t%d \t0x%04x \t0x%08x \t0x%x \t0x%x \t0x%x  \t0x%x\n",
									0, pAd->BcnRing.hw_desc_base, mbase[2], mcnt[2], mcidx[2], mdidx[2], mfidx[2]));

			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("\nBMCRing Configuration\n"));
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("\tRingIdx Reg \tBase \t\tCnt \tCIDX \tDIDX \tFIDX\n"));
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("\t%d \t0x%04x \t0x%08x \t0x%x \t0x%x \t0x%x  \t0x%x\n",
								0, pAd->TxBmcRing.hw_desc_base, mbase[3], mcnt[3], mcidx[3], mdidx[3], mfidx[3]));
		}
#endif /* MT_MAC */
 ...
 INT RTMPIoctlConnStatus(RTMP_ADAPTER *pAd, RTMP_STRING *arg)
{
	...
	BOOLEAN bConnect=FALSE;
 	...
 	{
 		CHAR ChStr[24] = {0};
 		switch(pAd->ApCfg.ApCliTab[ifIndex].ApcliFailReason) {
				case 0 :
 		...
 		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("ApCli%d Connected AP : Disconnect and reason=%s\n",ifIndex,ChStr));
 	}
 	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("=============================================================\n"));
     	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("<==RTMPIoctlConnStatus\n"));
