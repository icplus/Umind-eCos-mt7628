...
INT Set_BeaconReq_Proc(RTMP_ADAPTER *pAd, RTMP_STRING *arg)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("ChRepList="));
	for (ArgIdx=0; ArgIdx<MAX_NUM_OF_CHS; ArgIdx++)
	{
		if (BcnReq.ChRepList[ArgIdx] == 0)
			break;
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("%d ", \
		BcnReq.ChRepList[ArgIdx]));
	}
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("\n"));
	...
	IN MLME_QUEUE_ELEM *Elem) 
{
	...
	if (RRM_PeerNeighborReqSanity(pAd, Elem->Msg, Elem->MsgLen, &DialogToken, &pSsid, &SsidLen))
	{
		...
		ssidbuf[SsidLen]='\0';
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("pSsid=%s\n", /*pSsid*/ssidbuf));		
	...
	IN LONG Length)
{
	...
	*/
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Bssid=%02x:%02x:%02x:%02x:%02x:%02x, FrameType=%s\n",
			...
			case 1:
				if (BcnReqInfoField.field.ReportFrameType == 0) // 0 ==> beacon or probe response frame , 1 ==> Measurement Pilot frame
				{
					...
					bFrameBody = PeerBeaconAndProbeRspSanity(pAd,
								...
								FALSE,
								TRUE);
					MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s:: bFrameBody=%d\n", __FUNCTION__, bFrameBody));
				}
				break;
	...
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s():BcnReq->BSSID=%02x:%02x:%02x:%02x:%02x:%02x not equal ie_list->Bssid=%02x:%02x:%02x:%02x:%02x:%02x!\n",
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s():ie_list->Channel=%d\n",
