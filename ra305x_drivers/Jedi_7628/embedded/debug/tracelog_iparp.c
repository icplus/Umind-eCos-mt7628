...
static const CHECK_POINT_LIST tx_chk_pnt_list[] =
{
	...
	{P11, "rtmp_enq_req - enqueue failed"},
...
static const CHECK_POINT_LIST rx_chk_pnt_list[] =
{
	...
	{P7,  ""},
	{P8,  "Drop - Duplicate"},
	{P9,  "Drop - Old Received Packet"},
	{P10, "Drop - Already within reordering list"},
...
static const CODE_DESCRP eapol_code[] = {
		{FALSE ,  0, "(EAP-xxx)"},
};
...
/*******     IOCTL    *******/
VOID trace_iparp_print_usage(VOID)
{
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, 
		("\nUsage of set cmd: iwpriv <inf name> set <cmd name>=<value>\n"));
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, 
		("\t<cmd name>\n"
		 "\t\t trace_iparp_dbg_tx: debug contrl of tx path.\n"
		 "\t\t trace_iparp_dbg_rx: debug contrl of rx path.\n"
		 "\t<value>\n"
		 "\t\t Bit-wise hex value:\n"
		 "\t\t\t DONT_CHECK_TABLE = 1\n"
		 "\t\t\t DUMP_TABLE = 2\n"
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, 
		("\t<cmd name>\n"
		 "\t\t trace_iparp_tx_cri_chk_pnt: set tx critical check points.\n"
		 "\t\t trace_iparp_rx_cri_chk_pnt: set rx critical check points.\n"
		 "\t<value>\n"
		 "\t\t Bit-wise hex value.\n"
		 "\t\t Value of P0~P31, Px = (1 << x), ex. P0 = 1, P1 = 2, ...\n"));
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, 
		("\t<cmd name>\n"
		 "\t\t trace_iparp_tx_ign_pnt: set tx ignored points.\n"
		 "\t\t trace_iparp_rx_ign_pnt: set rx ignored points.\n"
		 "\t<value>\n"
		 "\t\t Bit-wise hex value.\n"
		 "\t\t Value of P0~P31, Px = (1 << x), ex. P0 = 1, P1 = 2, ...\n"));
}
...
INT trace_iparp_usage(
	IN	PRTMP_ADAPTER		ad,
	IN	RTMP_STRING			*arg)
{
	trace_iparp_print_usage();
	return TRUE;
}
	...
	IN	RTMP_STRING			*arg)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("%s(): dbg_ctrl = 0x%08x\n",
	...
	IN	RTMP_STRING			*arg)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("%s(): critical_chk_points = 0x%08x\n",
...
INT trace_iparp_set_tx_ignored_pnt(
	IN	PRTMP_ADAPTER		ad,
	IN	RTMP_STRING			*arg)
{
	PTRACE_TABLE_IPARP table = P_TRACE_TABLE_TX(ad);

	table->ignored_points = simple_strtol((RTMP_STRING *) arg, 0, 16);
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("%s(): ignored_points = 0x%08x\n",
				__FUNCTION__, table->ignored_points));

	return TRUE;
}


INT trace_iparp_set_rx_ignored_pnt(
	IN	PRTMP_ADAPTER		ad,
	IN	RTMP_STRING			*arg)
{
	PTRACE_TABLE_IPARP table = P_TRACE_TABLE_RX(ad);
...
INT trace_iparp_handle_dbg_ctrl(PTRACE_TABLE_IPARP table)
{
	...
	if (table->dbg_ctrl & TRACE_IPARP_DBG_TABLE_RESET)
...
INT trace_iparp_init_table(PRTMP_ADAPTER ad, PTRACE_TABLE_IPARP table, UCHAR type)
{
	...
	switch (type) 
	{
		...
		case RX_TRACE:
			...
			table->ignored_points = TRACE_IPARP_RX_IGN_PNTS;
			break;
		default:
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF,
			...
			"\tsize = %u\n"
			 ...
			 "\tlist_size = %x\n",
			  ...
			  table->critical_chk_points, table->ignored_points, table->list_size));
	...
	PTRACE_TABLE_IPARP_ENTRY entry)
{
	INT i;
	UINT32 mask;
	const CHAR* word[] = {"ARP", "ICMP", "REQ", "RSP","EAPOL", "n/a"};
	...
	if (entry->protocol_type == ETH_P_ARP) {
		...
		s_type = icmp_code[entry->icmparp_code].descrption;
#endif
	} else if (entry->protocol_type == /*ETH_TYPE_EAPOL*/0x888e) {
		s_protocol = word[4];
		s_type = eapol_code[entry->icmparp_code].descrption;		
	} else {
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR,
	...
	PTRACE_TABLE_IPARP_ENTRY entry)
{
	...
	*pass = TRUE; /* defaut */
	...
	if ((valid = entry->valid) == TRUE)
	{
		...
		if (entry->check_points & ign_pnts)
			ignored = TRUE;

	}
	NdisReleaseSpinLock(&table->lock);

	if (!(valid == FALSE || expired == FALSE) || ignored == TRUE)
	...
	PUCHAR	hdr_protype)
{
	...
	} else if (type_len == /*ETH_TYPE_EAPOL*/0x888e) {
		entry->protocol_type =  type_len;
		entry->icmparp_code = 0 ; // don't care
		entry->seq_num = 0; // don't care
	} else {
		{return TRACE_IPARP_NO_TRACE;}
	}
