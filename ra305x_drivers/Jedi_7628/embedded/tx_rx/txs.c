...
{	
		...
		)
	{
		...
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_INFO, ("%s():idx: %x, change state as idle\n", __FUNCTION__, bss_idx));
...
INT32 FtmTXSHandler(RTMP_ADAPTER *pAd, CHAR *Data)
{
	...
	BOOLEAN TxError = (txs_d0->RE || txs_d0->LE || txs_d0->ME);
	BOOLEAN TimerCancelled;

#ifdef RELEASE_EXCLUDE
	/* 	LE: Life Time error, pkt not transmitted.				*/
	/*	ME: MPDU error, transmission fail (reach retry limit)	*/
	/*	RE: RTS transmission fail (reach RTS retry limit) 		*/
#endif /* RELEASE_EXCLUDE */

	MTWF_LOG(DBG_CAT_PROTO, DBG_SUBCAT_ALL, DBG_LVL_WARN, 
		("   - FTM_TxS PID:0x%02X Err:%d%d%d\n", 
		txs_d4->pid, txs_d0->ME, txs_d0->RE, txs_d0->LE));

	pEntry = FtmGetPidPendingNode(pAd, txs_d4->pid);
...
#ifdef FTM_SUPPORT
	/* FtmTXSHandler */
	for (i = PID_FTM_MIN; i <= PID_FTM_MAX; i++) {
		AddTxSType(pAd, i, TXS_FORMAT0, FtmTXSHandler, FALSE); 
		TxSTypeCtl(pAd, i, TXS_FORMAT0, FALSE, TRUE);
	}
#endif /* FTM_SUPPORT*/
