	...
	IF_DEV_CONFIG_OPMODE_ON_AP(pAd)
	{
		...
		*/
...
#endif /* APCLI_SUPPORT */						
#ifdef FTM_SUPPORT
					if (IsPublicActionFrame(pAd, (VOID *)pHeader) == TRUE)
						bPassTheBcastPkt = TRUE; /* Let Public Action Frame pass */
#endif /* FTM_SUPPORT */
			...
			(pHeader->FC.Wep == 1) && (pRxInfo->Decrypted == 0))
		{
			...
			DefaultKeyIdx =pAd->ApCfg.MBSSID[apidx].wdev.DefaultKeyId;
	
			pKey=&(pAd->SharedKey[apidx][DefaultKeyIdx]);	

			/* handle WEP decryption */
			if (RTMPSoftDecryptWEP(pAd,
								   /*&pAd->SharedKey[pEntry->func_tb_idx][pRxBlk->key_idx],*/ //The pRxBlk->key_idx will wrong in IOT
									pKey,
								   pMgmt,
								   &mgmt_len) == FALSE)
			{
...
//+++Add by shiang for work-around, should remove it once we correctly configure the BSSID!
	...
	IF_DEV_CONFIG_OPMODE_ON_STA(pAd)
	{
		...
		{
#ifdef RELEASE_EXCLUDE
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_INFO,("CIPHER error = %d\n",pRxInfo->CipherErr));
#endif /* RELEASE_EXCLUDE */
	...
	IN struct wifi_dev *wdev)
{
	...
	if (!RTMPCheckWPAframe(pAd, pEntry, pData, data_len, wdev_idx, eth_frame))
	{
		...
		if (IS_ENTRY_CLIENT(pEntry) && (pEntry->PrivacyFilter == Ndis802_11PrivFilter8021xWEP))
		{
			...
			{
#ifdef WSC_AP_SUPPORT
				/* For Buffalo Windows STA, skip this deAuth rule due to:
				   1. send eapol start in 3s to AP after assocRsp for WPS.
				   2. STA send encrypt multicast Data before WPS done.
				 */
				if (pAd->ApCfg.MBSSID[pEntry->func_tb_idx].WscControl.WscConfMode != WSC_DISABLE)    
				{
					MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_WARN, ("==> Recv Encrypt Data before PortSecured, But in WPS & SKIP De-Auth this STA(%02x:%02x:%02x:%02x:%02x:%02x)\n", 
...
#ifdef RELEASE_EXCLUDE
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_INFO, ("!!! report BCAST DATA to LLC (len=%d) !!!\n", pRxBlk->DataSize));
#endif /* RELEASE_EXCLUDE */
		return;
	}
#endif /* CONFIG_STA_SUPPORT */
	...
	if (RTMP_CFG80211_MULTI_STA_ON(pAd, pAd->cfg80211_ctrl.multi_sta_net_dev) &&
		...
		if (MONITOR_ON(pAd) && pAd->monitor_ctrl.CurrentMonitorMode != MONITOR_MODE_OFF)
		{
			...
			else if(pAd->monitor_ctrl.CurrentMonitorMode == MONITOR_MODE_FULL)
			{
					...
					pClonePacket = ClonePacket(wdev->if_dev, rxblk.pRxPacket, rxblk.pData, rxblk.DataSize);
					rxblk.pRxPacket = pClonePacket;
					//sniffer_hex_dump("Clone Pkt content", GET_OS_PKT_DATAPTR(rxblk.pRxPacket), 32);
					STA_MonPktSend(pAd, &rxblk);
