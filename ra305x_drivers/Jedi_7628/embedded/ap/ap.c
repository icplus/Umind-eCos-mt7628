...
#ifdef MT_MAC
#if defined(FTM_SUPPORT) && defined(TMR_SUPPORT)
	MtSetTmrEnable(pAd, TMR_INITIATOR);
#endif
#endif /* MT_MAC */	
...
static VOID CheckApEntryInTraffic(RTMP_ADAPTER *pAd, MAC_TABLE_ENTRY *pEntry, STA_TR_ENTRY *tr_entry)
{
	...
	if((IS_ENTRY_APCLI(pEntry) || IS_ENTRY_CLIENT(pEntry))
		...
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("WICD%d, %dM, TxBytes:%d,  RxBytes:%d\n", pEntry->wcid, 
...
VOID ApLogEvent(RTMP_ADAPTER *pAd, UCHAR *pAddr, USHORT Event)
{
	if (pAd->EventTab.Num < MAX_NUM_OF_EVENT)
	{
		...
		pLog->Event = Event;
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("LOG#%ld %02x:%02x:%02x:%02x:%02x:%02x %s\n",
...

{
	...
	if ((needFallBack == FALSE)
		...
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("rollback the bandwidth setting to support 20/40Mhz\n"));
	}
