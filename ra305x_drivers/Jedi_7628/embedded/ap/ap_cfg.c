...
INT Set_LongRssiSet_Proc(RTMP_ADAPTER *pAd, RTMP_STRING *arg);
INT Set_MidRssiSet_Proc(RTMP_ADAPTER *pAd, RTMP_STRING *arg);
...
INT Set_TmrCal_Proc(PRTMP_ADAPTER pAd, RTMP_STRING *arg);
#endif /* TMR_SUPPORT */
...
static struct {
	...
	/*add by bo.jiao for long&Mid Sta Performence improve 20160217*/
	{"LongRssiSet",					Set_LongRssiSet_Proc},
	{"MidRssiSet",					Set_MidRssiSet_Proc},
...
/*	{"WnmMaxIdlePeriod",					Set_WnmMaxIdlePeriod_Proc}, */
    ...
    {"FtmNeverTO",				Set_FtmDisableFtmDurTimeout_Proc},
	...
	{"trace_iparp_tx_ign_pnt",			trace_iparp_set_rx_ignored_pnt},
	{"trace_iparp_rx_ign_pnt",			trace_iparp_set_rx_ignored_pnt},
...
#endif /* BAND_STEERING */
#ifdef TRACELOG_IPARP
	{"iparp_dbg_cmd",	trace_iparp_usage},
#endif /* TRACELOG_IPARP */
	{NULL,}
};
...
*/
		if(1)
			...
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("IOCTL::SIOCGIWAP(=%02x:%02x:%02x:%02x:%02x:%02x)\n",PRINT_MAC(pApCliEntry->MlmeAux.Bssid)));
            }
            else
            {
...
INT	Set_LongRssiSet_Proc(RTMP_ADAPTER *pAd, RTMP_STRING *arg)
 {
	...
	RssiSet = (int) simple_strtol(arg, 0, 10);

	pAd->CommonCfg.LongRssiSet = RssiSet;
	success = TRUE;
...
INT	Set_MidRssiSet_Proc(RTMP_ADAPTER *pAd, RTMP_STRING *arg)
 {
 	/*int  RssiSet,CcaH,CcaL;
	RTMP_STRING *token1,*token2;
	char sep = ':';
	
	The MidRssiSet inupt string format should be MidRssiSet:MidRssiCcaH:MidRssiCcaL,
	=>In MidRssiSet, if CCA < MidRssiCcaL, select MCS3,
	=>In MidRssiSet, if MidRssiCcaH < CCA < MidRssiCcaL, select MCS2,
	=>In MidRssiSet, if CCA > MidRssiCcaH, select MCS0,
	*/
	/*MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF,("%s():%s\n", __FUNCTION__, arg));
	token1 = strchr(arg, sep);
	if( token1 != NULL )
		token2 = strchr(token1+1, sep);
	
	if( token1 != NULL && token2 != NULL )
	{
		*token1 = '\0';
		*token2 = '\0';
		if (strlen(arg) && strlen(token1+1) && strlen(token2+1))
		{
			RssiSet = simple_strtol(arg, 0, 10);
			CcaH = simple_strtol(token1+1, 0, 10);
			CcaL = simple_strtol(token2+1, 0, 10);

			pAd->CommonCfg.MidRssiSet = RssiSet;
			pAd->CommonCfg.MidRssiCcaH = CcaH;
			pAd->CommonCfg.MidRssiCcaL = CcaL;
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("%s():Set MidRssiSet as(%d:%d:%d)\n",
						__FUNCTION__, RssiSet, CcaH , CcaL ));
			return TRUE;
		}
	}
		return FALSE;	
	...
	IN  RTMP_STRING *arg)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s : ifIndex=%u\n", __FUNCTION__,ifIndex));
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s: DE-AUTH request (Reason=%d)...\n", __FUNCTION__, Reason));
    ...
    IN  RTMP_STRING *arg)
{
    ...
    MtSetTmrEnable(pAd, (UCHAR)enable);
#endif /* MT_MAC */
