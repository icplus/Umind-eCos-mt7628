	...
	BOOLEAN	 Repeat)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("%s: %lx\n",__FUNCTION__, (ULONG)pTimer));
...
static VOID BndStrg_CancelTimer(RALINK_TIMER_STRUCT *pTimer, BOOLEAN *pCancelled)
{
	...
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_INFO,("%s() failed, Timer hasn't been initialize!\n", __FUNCTION__));
	}
...
INT BndStrg_PrintEntry(PBND_STRG_CLI_ENTRY entry)
{
...
#endif
	{
		...
		if (MAC_ADDR_EQUAL(table->MonitorAddr, entry->Addr))
			BND_STRG_MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, (YLW("*")));
#endif
		BND_STRG_MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF,
