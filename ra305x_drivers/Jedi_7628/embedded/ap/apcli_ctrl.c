...
//		if (pAd->CommonCfg.Channel != pAd->ScanTab.BssEntry[bss_idx].Channel)
	...
	*pCurrState = APCLI_CTRL_PROBE;
	...
	if( pAd->ApCfg.ApCliTab[ifIndex].ApcliFailReasonMaxTemp < pAd->ApCfg.ApCliTab[ifIndex].ApcliFailReasonMax )
		pAd->ApCfg.ApCliTab[ifIndex].ApcliFailReason = pAd->ApCfg.ApCliTab[ifIndex].ApcliFailReasonMaxTemp;
	else
		pAd->ApCfg.ApCliTab[ifIndex].ApcliFailReason = pAd->ApCfg.ApCliTab[ifIndex].ApcliFailReasonMax;
	
	pAd->ApCfg.ApCliTab[ifIndex].ApcliFailReasonMax = pAd->ApCfg.ApCliTab[ifIndex].ApcliFailReasonMaxTemp;
	pAd->ApCfg.ApCliTab[ifIndex].ApcliConnectState = APCLI_START_PROBE;
	pAd->ApCfg.ApCliTab[ifIndex].ApcliFailReasonMaxTemp = 0;
	...
	IN MLME_QUEUE_ELEM *Elem)
{
	...
	if (Status == MLME_SUCCESS)
	{
		...
		*pCurrState = APCLI_CTRL_AUTH;
		/*add for connection status*/
		pAd->ApCfg.ApCliTab[ifIndex].ApcliConnectState = APCLI_START_AUTH;
		pAd->ApCfg.ApCliTab[ifIndex].ApcliFailReasonMaxTemp = 1;
	...
	IN MLME_QUEUE_ELEM *Elem)
{
	...
	if(Status == MLME_SUCCESS)
	{
		...
		*pCurrState = APCLI_CTRL_ASSOC;
		/*add for connection status*/
		pAd->ApCfg.ApCliTab[ifIndex].ApcliConnectState = APCLI_START_ASSOC;
		pAd->ApCfg.ApCliTab[ifIndex].ApcliFailReasonMaxTemp = 2;
