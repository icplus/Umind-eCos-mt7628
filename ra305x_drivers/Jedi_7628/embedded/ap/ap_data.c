	...
	*/
	...
	if (tr_entry->PortSecured == WPA_802_1X_PORT_NOT_SECURED)
	{
		...
		)
		{
			...
			TRACE_IPARP_TX_CHK_PNT(pAd, RTMP_GET_PACKET_TRACEIPARP(pPacket), P1, 0);
		...
		if (pRxBlk->wcid < MaxWcidNum)
		{
			...
			if (pEntry && IS_ENTRY_APCLI(pEntry))
			{
				...
				{	
					...
					MTWF_LOG(DBG_CAT_AP, DBG_SUBCAT_ALL, DBG_LVL_ERROR,("Rx MIC Value error, Unicast!\n"));
					}
#endif /* APCLI_CERT_SUPPORT */
#endif /* WPA_SUPPLICANT_SUPPORT */ 
					MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR,("Rx MIC Value error\n"));
				}
			}
			else
#endif /* APCLI_SUPPORT */
		...
		else if (pRxInfo->Mcast || pRxInfo->Bcast)
		{
			...
			if (pEntry &&
				...
				if (pRxInfo->CipherErr == 2)
				{
					...
					MTWF_LOG(DBG_CAT_AP, DBG_SUBCAT_ALL, DBG_LVL_ERROR,("Rx MIC Value error, Bcast!\n"));
				}
			}
