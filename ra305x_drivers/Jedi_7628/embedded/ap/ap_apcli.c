...
//Eddy Todo
		...
		*/
		...
		COPY_MAC_ADDR(pApCliEntry->LastBssid, pAd->MlmeAux.Bssid);
			...
			if (pMacEntry->AuthMode >= Ndis802_11AuthModeWPA)
			{
				...
				pAd->ApCfg.ApCliTab[ifIndex].ApcliConnectState = APCLI_START_4WAY;
				pAd->ApCfg.ApCliTab[ifIndex].ApcliFailReasonMaxTemp = 3;
			}
			else
			{
				...
				/*add for connection status*/
				pAd->ApCfg.ApCliTab[ifIndex].ApcliConnectState = APCLI_CONNECTION_OK;
				pAd->ApCfg.ApCliTab[ifIndex].ApcliFailReasonMaxTemp = 4;
#ifdef __ECOS	
					{
						extern opmode;
						if (opmode == 2)
	...
	*/
		...
		(pAd->ApCfg.ApCliTab[ifIndex].WscControl.bWscTrigger == TRUE))
	{
		...
		//wsc_mjh_ssid = (PUCHAR)pApCliEntry->MlmeAux.Bssid;
		wsc_mjh_ssid = pAd->ApCfg.ApCliTab[ifIndex].Ssid;
	}
    else
    {
	mjh_wsc_fail = 1;
...
BOOLEAN ApCli_Close(RTMP_ADAPTER *pAd, PNET_DEV dev_p)
{
	...
	for (ifIndex = 0; ifIndex < MAX_APCLI_NUM; ifIndex++)
	{
		...
		{
			/* add for kill mlmetask failed when ifconfig apcli0 down */
