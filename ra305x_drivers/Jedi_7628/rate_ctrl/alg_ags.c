...
// Don't quick check within train down case
			...
			if (pAGSStatisticsInfo->TxErrorRatio > TrainDown)  // Poor quality
			{
				...
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_INFO | DBG_FUNC_RA, ("%s: QuickAGS: (DOWN) bad tx ok count (Current PER:%d, NewMcs's TrainDown:%d)\n", 
			...
			else
			{
				...
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_INFO | DBG_FUNC_RA, ("%s: QuickAGS: (Down) keep rate-down (Current PER:%d, NewMcs's TrainDown:%d)\n", 
					__FUNCTION__, pAGSStatisticsInfo->TxErrorRatio, TrainDown));
			}
#endif			
		}
