	...
	IN UCHAR			HwAggRateIndex)
{
	...
	if (Rate1ErrorRatio >= TrainDown)
	{
		...
		if (CurrRateIdx != DownRateIdx)
		{
			...
			/*If B/G MIX Mode but pCurrTxRate <= OFDM 9M ,not drop rate*/
			if( (Rssi > pAd->CommonCfg.MidRssiSet) && ( //(SUPPORT_HT_MODE|SUPPORT_OFDM_MODE|SUPPORT_CCK_MODE)=0x07
				((pEntry->SupportRateMode&0x07==0x07)&&((CurrRateIdx <= MCS_3)||(pCurrTxRate->Mode==MODE_OFDM)||(pCurrTxRate->Mode==MODE_CCK)))|| /* B/G/N MIX Mode */
				((pEntry->SupportRateMode&0x07==0x03)&&(CurrRateIdx <= MCS_3))/* B/G MIX Mode OFDM 9M*/) )
			{		
				pEntry->CurrTxRateIndex = MCS_3;
				if( CurrRateIdx == MCS_3 )
					pEntry->LastSecTxRateChangeAction = RATE_NO_CHANGE;
				else
					pEntry->LastSecTxRateChangeAction = RATE_UP;
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_WARN,("%s;Rssi[%d] > %d and CurrMCS <= MCS_3;pCurrTxRate->CurrMCS=%d,CurrRateIdx=%d\n",__FUNCTION__,Rssi,pAd->CommonCfg.MidRssiSet,pCurrTxRate->CurrMCS,CurrRateIdx));
			}
			/*Bo.jiao add for 360  promote long sta performance 20160114*/
			/*If B/G/N MIX Mode but pCurrTxRate <= MCS_0, ,not drop rate*/
			/*If B/G MIX Mode but pCurrTxRate <= CCK 2M ,not drop rate*/
			else if((Rssi > pAd->CommonCfg.LongRssiSet) && 
				   ((pEntry->SupportRateMode&0x07==0x07)&&((CurrRateIdx <= MCS_0)||(pCurrTxRate->Mode==MODE_OFDM)||(pCurrTxRate->Mode==MODE_CCK)))/* B/G/N MIX Mode */)
			{
				pEntry->CurrTxRateIndex = MCS_0;
				if( CurrRateIdx == MCS_0 )
					pEntry->LastSecTxRateChangeAction = RATE_NO_CHANGE;
				else
					pEntry->LastSecTxRateChangeAction = RATE_UP;
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_WARN,("%s;Rssi[%d] > %d, BGN MCS0 -> OFDM/CCK,pCurrTxRate->CurrMCS=%d,CurrRateIdx=%d\n",__FUNCTION__,Rssi,pAd->CommonCfg.LongRssiSet,pCurrTxRate->CurrMCS,CurrRateIdx));
			}
			else if((Rssi > pAd->CommonCfg.LongRssiSet) && 
				   ((pEntry->SupportRateMode&0x07==0x03)&&(CurrRateIdx <= MCS_1))/* B/G MIX Mode CCK2M*/	)
			{
				pEntry->CurrTxRateIndex = MCS_1;
				if( CurrRateIdx == MCS_1 )
					pEntry->LastSecTxRateChangeAction = RATE_NO_CHANGE;
				else
					pEntry->LastSecTxRateChangeAction = RATE_UP;
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_WARN,("%s;Rssi[%d] > %d, BG CCK 2M -> CCK 1M,pCurrTxRate->CurrMCS=%d,CurrRateIdx=%d\n",__FUNCTION__,Rssi,pAd->CommonCfg.LongRssiSet,pCurrTxRate->CurrMCS,CurrRateIdx));
			}
			/*other case,keep the original algorithm*/
			else
			{
				pEntry->CurrTxRateIndex = DownRateIdx;
				pEntry->LastSecTxRateChangeAction = RATE_DOWN;
		...
		else 
		{
			...
			{		
				...
				if( CurrRateIdx == MCS_3 )
					...
					pEntry->LastSecTxRateChangeAction = RATE_UP;
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_WARN,("%s;Rssi[%d] > %d, go back CurrMCS <= MCS_3;pCurrTxRate->CurrMCS=%d,CurrRateIdx=%d\n",__FUNCTION__,Rssi,pAd->CommonCfg.MidRssiSet,pCurrTxRate->CurrMCS,CurrRateIdx));
			}
			/*Bo.jiao add for 360  promote long sta performance 20160114*/
			/*If B/G/N MIX Mode but pCurrTxRate <= MCS_0, go back to MCS_0*/
			/*If B/G MIX Mode but pCurrTxRate <= CCK 2M ,go back to CCK 2M*/
			else if((Rssi > pAd->CommonCfg.LongRssiSet) && 
				   ((pEntry->SupportRateMode&0x07==0x07)&&((CurrRateIdx <= MCS_0)||(pCurrTxRate->Mode==MODE_OFDM)||(pCurrTxRate->Mode==MODE_CCK)))/* B/G/N MIX Mode */)
			{
				pEntry->CurrTxRateIndex = MCS_0;
				if( CurrRateIdx == MCS_0 )
					...
					pEntry->LastSecTxRateChangeAction = RATE_UP;
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_WARN,("%s;Rssi[%d] > %d, BGN go back OFDM/CCK -> MCS0,pCurrTxRate->CurrMCS=%d,CurrRateIdx=%d\n",__FUNCTION__,Rssi,pAd->CommonCfg.LongRssiSet,pCurrTxRate->CurrMCS,CurrRateIdx));
			}
			else if((Rssi > pAd->CommonCfg.LongRssiSet) && 
				   ((pEntry->SupportRateMode&0x07==0x03)&&(CurrRateIdx <= MCS_1))/* B/G MIX Mode CCK2M*/	)
			{
				...
				if( CurrRateIdx == MCS_1 )
					...
					pEntry->LastSecTxRateChangeAction = RATE_UP;
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_WARN,("%s;Rssi[%d] > %d, BG go back CCK 1M -> CCK 2M,pCurrTxRate->CurrMCS=%d,CurrRateIdx=%d\n",__FUNCTION__,Rssi,pAd->CommonCfg.LongRssiSet,pCurrTxRate->CurrMCS,CurrRateIdx));
			}
			/*other case,do not deal*/
...
#ifdef RELEASE_EXCLUDE
/*
	Low Packet Algorithm
		1.	Down MCS Rate : "Error / Success Ratio" > "3 Down MCS Rate" with larger than X Confidence Counter								
		2.	Up MCS Rate : "Error / Success Ratio" < "2 Down MCS Rate" with larger than X Confidence Counter								
		3.	"Where : X Confidence Counter mean getting 10 packets, NOT RA Slot Time
			If More Packet Number, response time is shorter"
*/
#endif /* RELEASE_EXCLUDE */
...
#if 0
	if (pAd->dbg_ctrl & 0x800)
	{
	printk("\033[1;36m %s, %u, OneSecTxNoRetryOkCount = %u \033[0m\n", __FUNCTION__, __LINE__, pEntry->OneSecTxNoRetryOkCount);  /* Haipin Debug Print (B)*/
	printk("\033[1;36m %s, %u, OneSecTxRetryOkCount = %u \033[0m\n", __FUNCTION__, __LINE__, pEntry->OneSecTxRetryOkCount);  /* Haipin Debug Print (B)*/
	printk("\033[1;36m %s, %u, OneSecTxFailCount = %u \033[0m\n", __FUNCTION__, __LINE__, pEntry->OneSecTxFailCount);  /* Haipin Debug Print (B)*/
	printk("\033[1;36m %s, %u, TxSuccCount = %u, ErrToSuccRatio = %u\033[0m\n", __FUNCTION__, __LINE__, TxSuccCount, ErrToSuccRatio);  /* Haipin Debug Print (B)*/
	}
#endif
	...
	if (pAd->dbg_ctrl & 0x800)
	{
	printk("\033[1;32m %s, %u, NewTxRateIdx = %u \033[0m\n", __FUNCTION__, __LINE__, NewTxRateIdx);  /* Haipin Debug Print (B)*/
	}
#endif
...
#if 0
	if (pAd->dbg_ctrl & 0x1000)
		printk("\033[1;31m %s, %u \033[0m\n", __FUNCTION__, __LINE__);  /* Haipin Debug Print (R) */
#endif
...
#if 0
VOID Dump_Debg_RaInfo(RA_INFO *ra_info)
{
	MT_TX_COUNTER *pTxInfo = &ra_info->TxInfo;

	printk("TxCount = %u; TxFailCount = %u; Rate1TxCnt = %u; Rate1FailCnt = %u; Rate2TxCnt = %u; Rate3TxCnt = %u; Rate4TxCnt = %u; Rate5TxCnt = %u; RateIndex = %u\n",
		   pTxInfo->TxCount, pTxInfo->TxFailCount, pTxInfo->Rate1TxCnt, pTxInfo->Rate1FailCnt, pTxInfo->Rate2TxCnt, pTxInfo->Rate3TxCnt, pTxInfo->Rate4TxCnt, pTxInfo->Rate5TxCnt, pTxInfo->RateIndex);
	printk("lowTrafficCount = %u; mcsGroup = %u; TrafficLoading:%u-->%u; RaHoldTime = %u; CurrentTxRateIndex = %u; lastRateIdx = %u\n", 
		   ra_info->lowTrafficCount, ra_info->mcsGroup, ra_info->TrafficLoadingOld, ra_info->TrafficLoading, ra_info->RaHoldTime, ra_info->CurrTxRateIndex, ra_info->lastRateIdx);
}
#endif
...
#if 0 /* zeoro traffic should just use rssi mapping (after hold time) */
...
#ifdef RELEASE_EXCLUDE
			/* 
				in this case, lastRateIdx and CurrTxRateIndex are already updated in NewRateAdaptMT().
				so, just go to end_of_ra.
				For now, moving them out is too risky.
			*/
#endif /* RELEASE_EXCLUDE */
			goto end_of_ra;
			break;
...
#if 0 /* Debug */
	pEntry->dbg_ra_info[pEntry->dbg_ra_info_index].CurrTxRateIndex = pEntry->CurrTxRateIndex;
	pEntry->dbg_ra_info[pEntry->dbg_ra_info_index].lastRateIdx= pEntry->lastRateIdx;
	pEntry->dbg_ra_info[pEntry->dbg_ra_info_index].RaHoldTime= pEntry->RaHoldTime;
	pEntry->dbg_ra_info[pEntry->dbg_ra_info_index].TrafficLoading = pEntry->TrafficLoading;
	pEntry->dbg_ra_info[pEntry->dbg_ra_info_index].TrafficLoadingOld= TrafficLoadingOld;
	pEntry->dbg_ra_info[pEntry->dbg_ra_info_index].mcsGroup= pEntry->mcsGroup;
	pEntry->dbg_ra_info[pEntry->dbg_ra_info_index].lowTrafficCount= pEntry->lowTrafficCount;
	NdisCopyMemory(&pEntry->dbg_ra_info[pEntry->dbg_ra_info_index].TxInfo, &TxInfo, sizeof(MT_TX_COUNTER));

	if (pAd->dbg_ctrl & 0x400)
		Dump_Debg_RaInfo(&pEntry->dbg_ra_info[pEntry->dbg_ra_info_index]);

	if (pEntry->dbg_ra_info_index < 30)
		pEntry->dbg_ra_info_index++;
	else
		pEntry->dbg_ra_info_index = 0;
#endif
